...
TEST_F(SerializeTraitsTest, can_serialize_to_array) {
    ...
    char array[size];
    ASSERT_TRUE(Serialization::serialize_to_array_with_cached_size(s, array, size));
    S ss {gen};
    ASSERT_TRUE(Serialization::parse_from_array(array, size, ss));
    ASSERT_EQ(s.v, ss.v);
}

TEST_F(SerializeTraitsTest, calculate_size_before_serialize_when_cache_is_needed) {
    struct S : public MinimalSerializable {
        void serialize(CodedOutputStream& os) const {
            calculate_serialized_size_before_serialize = calculate_serialized_size_called;
