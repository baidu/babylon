...
struct Normal {
    Normal(int32_t) {construct_num++;}
    Normal(const Normal&) {
        copy_num++;
        construct_num++;
    }
    Normal(Normal&&) {move_num++;construct_num++;}
    virtual ~Normal() {destruct_num++;}
...
TEST(any, static_initialize_works_fine) {
    ...
    ASSERT_EQ(static_instance.pstring, static_instance.string.get<::std::string>());
    ASSERT_NE(0, static_instance.primitive.as<uint64_t>());
    ASSERT_EQ(static_instance.primitive_value, static_instance.primitive.as<uint64_t>());
