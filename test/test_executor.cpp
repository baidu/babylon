...
TEST(executor, can_execute_binded_function) {
    int value {1};
    auto future = inplace_executor.execute(
        ::std::bind([&] (int add, int mul) {
            return value = value * mul + add;
        }, 1, ::std::placeholders::_1), 10086);
    ASSERT_TRUE(future.valid());
    ASSERT_TRUE(future.ready());
    ASSERT_EQ(10087, future.get());
    future = async(inplace_executor,
...
TEST(executor, handle_non_copyable_function_and_args) {
    ...
    future = inplace_executor.execute([] (S&& s, int& value) {
        return s(value);
    }, S {6}, ::std::ref(value));
    ASSERT_TRUE(future.valid());
    ASSERT_TRUE(future.ready());
    ASSERT_EQ(10108, future.get());
    future = async(inplace_executor, [] (S&& s, int& value) {
        return s(value);
    }, S {7}, ::std::ref(value));
    ASSERT_TRUE(future.valid());
    ASSERT_TRUE(future.ready());
    ASSERT_EQ(10115, future.get());
    inplace_executor.submit([] (S&& s, int& value) {
        return s(value);
    }, S {8}, ::std::ref(value));
    ...
    for (size_t i = 0; i < concurrent; ++i) {
        level1_futures[i] = executor.execute([&, i] {
            ...
            for (size_t j = 0; j < times; ++j) {
                ...
                level2_futures[i * times + j] = executor.execute([] (::std::string&& s) {
                    return s.size();
                }, ::std::move(s));
                sum += i * times + j;
            }
