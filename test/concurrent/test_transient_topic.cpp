...
TEST(concurrent_transient_topic, consume_stop_before_full_batch_get_small_range) {
    ConcurrentTransientTopic<::std::string> topic;
    auto consumer = topic.subscribe();
    ::std::promise<ConcurrentTransientTopic<::std::string>::ConsumeRange> promise;
    auto future = promise.get_future();
...
TEST(concurrent_transient_topic, const_topic_consume_const_item) {
    ...
    topic.publish("10086");
    topic.publish("10010");
    auto consumer = static_cast<const ConcurrentTransientTopic<::std::string>&>(topic).subscribe();
...
TEST(concurrent_transient_topic, support_concurrent_publish_consume) {
    ...
    size_t expect_sum = consume_concurrents * (publish_concurrents - 1) * publish_concurrents / 2;
