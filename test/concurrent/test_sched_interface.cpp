...
TEST(futex_interface, waiter_wakeup_one_by_one) {
    ...
    auto wait_func = [&] {
        while (futex.load(::std::memory_order_relaxed) == 0) {
            SchedInterface::futex_wait(
                &reinterpret_cast<uint32_t&>(futex), 0, nullptr);
        }
    ...
    ASSERT_EQ(1, SchedInterface::futex_wake_one(
        &reinterpret_cast<uint32_t&>(futex)));
    ASSERT_EQ(::std::future_status::timeout, future.wait_for(::std::chrono::milliseconds(100)));
    ASSERT_EQ(1, running.load(::std::memory_order_relaxed));
    ASSERT_EQ(1, SchedInterface::futex_wake_one(
        &reinterpret_cast<uint32_t&>(futex)));
    //ASSERT_EQ(::std::future_status::ready, future.wait_for(::std::chrono::milliseconds(100)));
    f1.get();
    f2.get();
...
TEST(futex_interface, waiter_wakeup_all) {
    {
        ...
        ASSERT_EQ(::std::future_status::timeout, f1.wait_for(::std::chrono::milliseconds(100)));
