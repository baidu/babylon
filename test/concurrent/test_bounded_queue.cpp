...
TEST(concurrent_bounded_queue, try_pop_wakeup_blocking_push) {
    ...
    ASSERT_EQ(::std::future_status::timeout, finished.wait_for(::std::chrono::milliseconds(100)));
...
TEST(concurrent_bounded_queue, try_pop_n_like_normal_batch_before_end) {
    ...
    ASSERT_EQ(2, queue.size());
    v.clear();
    queue.try_pop_n<true, true>([&] (IT iter, IT end) {
...
TEST(concurrent_bounded_queue, try_pop_n_cut_to_end) {
    ...
    v.clear();
    auto ret = queue.try_pop_n<true, true>([&] (IT iter, IT end) {
        ::std::copy(iter, end, ::std::back_inserter(v));
    }, 4);
    ASSERT_EQ(3, ret);
    ASSERT_EQ(3, v.size());
...
TEST(concurrent_bounded_queue, press_blocking_mpsc_with_try_pop) {
    ...
    size_t total = batch_size * batch_concurrent * times + single_concurrent * times;
    ::std::vector<::std::future<size_t>> pop_futures;
    pop_futures.emplace_back(::std::async(::std::launch::async, [&] {
        size_t sum = 0;
        size_t count = 0;
...
TEST(concurrent_bounded_queue, press_spining_mpsc_with_try_pop) {
    ...
    pop_futures.emplace_back(::std::async(::std::launch::async, [&] {
        ...
        size_t count = 0;
        while (count < total) {
            size_t poped = queue.try_pop_n<false, false>([&] (IT iter, IT end) {
                while (iter != end) {
                    sum += *iter++;
                }
            }, batch_size);
