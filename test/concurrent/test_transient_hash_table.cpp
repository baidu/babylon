...
TEST(fixed_swiss_table, iterable) {
    ...
    auto& const_table = static_cast<
        const ConcurrentFixedSwissTable<::std::string>&>(table);
    sum = 0;
...
TEST(hash_set, bucket_count_ceil_to_pow2) {
    auto min_bucket_count = ConcurrentTransientHashSet<::std::string>{}.bucket_count();
    for (size_t i = 0; i <= min_bucket_count; ++i) {
        ASSERT_EQ(min_bucket_count, ConcurrentTransientHashSet<::std::string>{i}.bucket_count());
    }
    for (size_t i = min_bucket_count + 1; i <= 2 * min_bucket_count; ++i) {
        ASSERT_EQ(2 * min_bucket_count, ConcurrentTransientHashSet<::std::string>{i}.bucket_count());
    }
    ASSERT_EQ(4 * min_bucket_count, ConcurrentTransientHashSet<::std::string>{2 * min_bucket_count + 1}.bucket_count());
}
...
TEST(hash_set, iterable) {
    ...
    ASSERT_EQ(expected, sum);
    auto& const_set = static_cast<
        const ConcurrentTransientHashSet<::std::string>&>(set);
    sum = 0;
...
TEST(hash_map, move_constructible) {
    ConcurrentTransientHashMap<::std::string, ::std::string> map;
    map.emplace("10086", "10010");
    ConcurrentTransientHashMap<::std::string, ::std::string> moved_map(::std::move(map));
