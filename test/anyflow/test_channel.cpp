...
struct ChannelOutputProcessor : public GraphProcessor {
    ...
    ANYFLOW_INTERFACE(
        ANYFLOW_EMIT_CHANNEL(::std::string, x)
    )
};
::std::promise<OutputChannel<::std::string>> ChannelOutputProcessor::enter_promise;
::std::future<OutputChannel<::std::string>> ChannelOutputProcessor::enter_future;
...
struct ChannelInputProcessor : public GraphProcessor {
    ...
    ANYFLOW_INTERFACE(
        ANYFLOW_DEPEND_CHANNEL(::std::string, a)
    )
};
::std::promise<ChannelConsumer<::std::string>> ChannelInputProcessor::enter_promise;
::std::future<ChannelConsumer<::std::string>> ChannelInputProcessor::enter_future;
...
struct MutableChannelInputProcessor : public GraphProcessor {
    ...
    ANYFLOW_INTERFACE(
        ANYFLOW_DEPEND_MUTABLE_CHANNEL(::std::string, a)
    )
};
::std::promise<MutableChannelConsumer<::std::string>> MutableChannelInputProcessor::enter_promise;
::std::future<MutableChannelConsumer<::std::string>> MutableChannelInputProcessor::enter_future;
    ...
    virtual void SetUp() override {
        ...
        builder.set_executor(executor);
        {
            auto& vertex = builder.add_vertex([] {
                return ::std::unique_ptr<ChannelOutputProcessor>(new ChannelOutputProcessor);
            });
            vertex.named_emit("x").to("A");
        }
        {
            auto& vertex = builder.add_vertex([] {
                return ::std::unique_ptr<ChannelInputProcessor>(new ChannelInputProcessor);
            });
        ...
        {
            auto& vertex = builder.add_vertex([] {
                return ::std::unique_ptr<MutableChannelInputProcessor>(new MutableChannelInputProcessor);
            });
...
TEST_F(ChannelTest, data_ready_after_channel_open) {
    ...
    ASSERT_EQ(::std::future_status::timeout, ChannelInputProcessor::enter_future.wait_for(::std::chrono::milliseconds(100)));
...
TEST_F(ChannelTest, publisher_close_channel_when_destruct) {
    ...
    ChannelConsumer<::std::string> consumer;
    {
        ...
        ASSERT_EQ(::std::future_status::timeout, future.wait_for(::std::chrono::milliseconds(100)));
    }
    future.get();
    ChannelOutputProcessor::leave_promise.set_value();
    ChannelInputProcessor::leave_promise.set_value();
