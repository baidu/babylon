...
struct ConstProcessor : public GraphProcessor {
    ...
    ANYFLOW_INTERFACE(
        ANYFLOW_DEPEND_DATA(::std::string, a, 1)
...
struct VariableProcessor : public GraphProcessor {
    ...
    ANYFLOW_INTERFACE(
        ANYFLOW_DEPEND_VA_DATA(::std::string, dv)
...
TEST_F(DependencyTest, anonymous_immediately_ready_when_target_ready_and_condition_established) {
    *c4->emit<bool>() = true;
...
TEST_F(DependencyTest, anonymous_immediately_ready_when_condition_not_established) {
    *c4->emit<bool>() = false;
    ::std::vector<Closure> cv;
    for (size_t index = 0; index < grv.size(); index++) {
        cv.emplace_back(graph->run(grv[index]));
