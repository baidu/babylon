...
TEST(ExclusiveMonotonicBufferResource, allocate_with_alignment) {
    ...
    for (size_t i = 0; i < 10; ++i) {
        ASSERT_EQ(0, reinterpret_cast<uintptr_t>(resource.allocate<8192>(1)) % 8192);
    }
}
...
TEST(ExclusiveMonotonicBufferResource, allocate_oversize_with_alignment) {
    ...
    for (size_t i = 0; i < 10; ++i) {
        ASSERT_EQ(0, reinterpret_cast<uintptr_t>(resource.allocate<8192>(8000)) % 8192);
    }
}
...
TEST(ExclusiveMonotonicBufferResource, moveable) {
    ...
    for (size_t i = 0; i < 10; ++i) {
        auto s = reinterpret_cast<::std::string*>(resource.allocate<alignof(::std::string)>(sizeof(::std::string)));
...
TEST(ExclusiveMonotonicBufferResource, destructor_call_when_release_in_reverse_order) {
    struct S {
        ~S() {
    ...
    for (size_t i = 0; i < 10; ++i) {
        threads.emplace_back([&] {
            for (size_t j = 0; j < 1000; ++j) {
                ...
                resource.register_destructor(s);
                s = reinterpret_cast<::std::string*>(resource.allocate<alignof(::std::string)>(sizeof(::std::string)));
