...
TEST_F(ReusableVectorTest, trivial_as_element) {
  ...
  ASSERT_TRUE(
      ::std::is_trivially_destructible<SwissVector<SwissString>>::value);
  struct S {
    ~S() {}
  } s;
...
TEST_F(ReusableVectorTest, elements_uses_allocator) {
  ...
  vvs.emplace_back(
      ::std::initializer_list<::std::string> {long_string + "10086"});
  auto& vs = vvs.back();
  ...
  vs.assign(
      {long_string + "10010", long_string + "10011", long_string + "10012"});
  ASSERT_EQ(3, vs.size());
  ASSERT_TRUE(resource.contains(&vs[0]));
...
TEST_F(ReusableVectorTest, erase_keep_instance) {
  ...
  vs.insert(vs.begin(),
            {"10016", "8610010", "10086", "8610016", "10010", "8610086"});
...
TEST_F(ReusableVectorTest, reusable) {
  ...
  auto& rvs = *Reuse::create_with_allocation_metadata<SwissVector<SwissString>>(
      allocator, meta);
...
struct VectorReusableSerializable {
  ...
  VectorReusableSerializable(allocator_type allocator)
      : vs {allocator}, ss {allocator} {}
