...
TEST_F(ReusableTraitsTest, class_with_dynamic_content_can_be_reusable) {
    struct S {
        ...
        //S(const char* str, allocator_type allocator) : s {str, str + strlen(str), allocator} {}
        S(const AllocationMetadata& meta, allocator_type allocator) : s {allocator} {
...
#if __cpp_lib_memory_resource >= 201603L && (!__GLIBCXX__ || _GLIBCXX_USE_CXX11_ABI)
...
#endif // __cpp_lib_memory_resource >= 201603L && (!__GLIBCXX__ || _GLIBCXX_USE_CXX11_ABI)
...
TEST_F(ReusableTraitsTest, scala_vector_is_reusable) {
    ...
    {
        auto& ss = *Reuse::create_with_allocation_metadata<::std::vector<int>>(allocator, meta);
        ASSERT_EQ(10086, ss.capacity());
        ASSERT_TRUE(ss.empty());
    }
}
...
TEST_F(ReusableTraitsTest, message_reusable) {
    ...
    {
        auto& mm = *Reuse::create_with_allocation_metadata<TestMessage>(allocator, meta);
