...
TEST_F(LogInterfaceTest, change_log_backend) {
    LogInterface::set_provider(::std::unique_ptr<LogStreamProvider>(new MockLogStreamProvider));
    auto file = __FILE__;
    auto before_line = __LINE__;
    BABYLON_LOG(INFO) << "this line should appear in provider";
    auto after_line = __LINE__;
    MockLogStreamProvider& provider = dynamic_cast<MockLogStreamProvider&>(LogInterface::provider());
    ASSERT_NE(::std::string::npos, provider.ss.str().find("this line should appear in provider"));
