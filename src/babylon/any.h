...
public:
    ...
    template <typename T, typename ::std::enable_if<sizeof(uint64_t) < sizeof(T), int>::type = 0>
    ...
    // 常量取值
    template <typename T, typename ::std::enable_if<sizeof(size_t) < sizeof(T), int>::type = 0>
    inline const T* cget() const noexcept;
    // 特化支持小对象内联存储
    template <typename T, typename ::std::enable_if<sizeof(T) <= sizeof(size_t), int>::type = 0>
    inline const T* cget() const noexcept;
    template <typename T>
...
private:
    ...
    template <typename T>
    struct TypeDescriptor<T, typename ::std::enable_if<::std::is_copy_constructible<T>::value>::type> :
    ...
    template <typename T, typename ::std::enable_if<
        ...
        (::std::is_array<T>::value ||
         ::std::is_function<T>::value), int>::type = 0>
    inline void construct_inplace(T&& value) noexcept;
    template <typename T, typename ::std::enable_if<
        !::std::is_integral<T>::value &&
        !::std::is_array<T>::value &
