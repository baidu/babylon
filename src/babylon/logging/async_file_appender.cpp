...
void LogStreamBuffer::overflow_page_table() noexcept {
    // 当前页表用尽，需要分配一个新的页表
    auto page_table = reinterpret_cast<Log::PageTable*>(_page_allocator->allocate());
    page_table->next = nullptr;
    // 计算新页表的起止点
    auto pages = page_table->pages;
    auto pages_end = reinterpret_cast<char**>(
            reinterpret_cast<uintptr_t>(page_table) +
    ...
    if (_pages == _log.pages + Log::INLINE_PAGE_CAPACITY) {
        ...
        *pages++ = reinterpret_cast<char*>(_log.head);
        ...
        auto last_page_table = reinterpret_cast<Log::PageTable*>(
                reinterpret_cast<uintptr_t>(_pages_end) -
                _page_allocator->page_size());
...
void AsyncFileAppender::set_page_allocator(PageAllocator& page_allocator) noexcept {
...
int AsyncFileAppender::initialize() noexcept {
    ...
    _extend_page_capacity =
        (_page_allocator->page_size() - sizeof(Log::PageTable)) /
        sizeof(char*);
...
void AsyncFileAppender::keep_writing() noexcept {
    ...
    do {
        ...
        auto poped = _queue.try_pop_n<false, false>([&] (Queue::Iterator iter, Queue::Iterator end) {
            while (iter < end) {
                stop = collect_log_pages(*iter++, writev_payloads, pages_to_release);
            }
        }, batch);
...
bool AsyncFileAppender::collect_log_pages(
    ...
    if (data_page_num > Log::INLINE_PAGE_CAPACITY) {
        ...
        collect_log_pages(*log.head, remain,
                          writev_payloads, pages_to_release);
        return false;
    }
...
void AsyncFileAppender::collect_log_pages(Log::PageTable& extend_log, size_t size,
    ...
    while (remain >= extend_capacity) {
        collect_full_pages(extend_log_ptr->pages, extend_page_capacity,
                           payloads, pages);
    ...
    // 收集尾部可能不完整的页表
    if (remain > 0) {
        collect_pages(extend_log_ptr->pages, remain,
                      payloads, pages);
        pages.emplace_back(extend_log_ptr);
    }
}
...
void AsyncFileAppender::collect_full_pages(
    ...
    for (size_t i = 0; i < num; ++i) {
        auto page = pages[i];
        writev_payloads.emplace_back(::iovec{.iov_base = page,
                                             .iov_len = page_size});
...
void AsyncFileAppender::write_to_fd(
        ::std::vector<::iovec>& writev_payloads,
        int fd) noexcept {
    size_t begin_index = 0;
    do {
        auto size = ::std::min<size_t>(UIO_MAXIOV, writev_payloads.size() - begin_index);
