...
public:
    ...
    execute(C&& callable, Args&&... args) noexcept;
...
private:
    ...
    inline static auto normalize(C&& callable) noexcept -> decltype(::std::mem_fn(callable));
    ...
    template <typename R, typename F, typename C, typename... Args, typename ::std::enable_if<
        !::std::is_same<void, R>::value, int>::type = 0>
    inline static void run_and_set(Promise<R, F>& promise, C&& callable, Args&&... args) noexcept;

    // void特化
    template <typename F, typename C, typename... Args>
    inline static void run_and_set(Promise<void, F>& promise, C&& callable, Args&&... args) noexcept;
};

// 仿照std::async的函数版本，executor替代了内置的policy机制，可用户扩展
template <typename F = SchedInterface, typename C, typename... Args>
inline Future<typename InvokeResult<C, Args...>::type, F>
async(Executor& executor, C&& callable, Args&&... args) noexcept {
    return executor.execute<F>(::std::forward<C>(callable), ::std::forward<Args>(args)...);
}
