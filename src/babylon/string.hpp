...
inline void STLStringResizeUninitialized(::std::basic_string<C, T, A>* string,
    ...
    // string operator[]会尝试调整string的copy on write状态，有额外开销，采用const_cast规避
...
inline void stable_reserve(
        ...
        typename ::std::basic_string<C, T, A>::size_type
            min_capacity) noexcept {
    using StringType = ::std::basic_string<C, T, A>;
...
#endif // __GLIBCXX__ && !_GLIBCXX_USE_CXX11_ABI && GLIBCXX_VERSION >= 720191114L
