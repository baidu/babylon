...
inline Future<typename InvokeResult<C, Args...>::type, F>
Executor::execute(C&& callable, Args&&... args) noexcept {
    using DC = typename ::std::decay<C>::type;
    using R = typename InvokeResult<C, Args...>::type;
    Promise<R, F> promise;
    auto future = promise.get_future();
    MoveOnlyFunction<void(void)> function {
        ::std::bind([] (Promise<R, F>& promise, DC& callable,
                    typename ::std::decay<Args>::type&... args) {
                run_and_set(promise, normalize(::std::move(callable)), ::std::move(args)...);
            }, ::std::move(promise),
...
inline int Executor::submit(C&& callable, Args&&... args) noexcept {
    ...
    MoveOnlyFunction<void(void)> function {
        ::std::bind([] (DC& callable, typename ::std::decay<Args>::type&... args) {
...
inline auto Executor::normalize(C&& callable) noexcept -> decltype(::std::mem_fn(callable)) {
...
template <typename F, typename C, typename... Args>
inline void Executor::run_and_set(Promise<void, F>& promise, C&& callable, Args&&... args) noexcept {
    callable(::std::forward<Args>(args)...);
    promise.set_value();
}

template <typename R, typename F, typename C, typename... Args, typename ::std::enable_if<
    !::std::is_same<void, R>::value, int>::type>
inline void Executor::run_and_set(Promise<R, F>& promise, C&& callable, Args&&... args) noexcept {
