...
inline void ConcurrentBoundedQueue<T, S>::
SlotFutex::wait_until_reach_expected_version(
    ...
    if (USE_FUTEX_WAIT) {
        block_until_reach_expected_version_slow(
            current_version_and_waiters, expected_version, timeout, order);
    } else {
        spin_until_reach_expected_version_slow(
            current_version_and_waiters, expected_version, timeout, order);
    }
}
...
inline void ConcurrentBoundedQueue<T, S>::SlotFutex::set_version(
    ...
    reinterpret_cast<::std::atomic<uint16_t>*>(&_futex.value())->store(
            version, order);
}
...
inline void ConcurrentBoundedQueue<T, S>::SlotFutex::wakeup_waiters(
    ...
    // 检测是否需要执行唤醒
    auto current_version_and_waiters = _futex.value().load(::std::memory_order_relaxed);
...
template <typename T, typename S>
inline void ConcurrentBoundedQueue<T, S>::
SlotFutex::set_version_and_wakeup_waiters(
        uint16_t next_version) noexcept {
    auto current_version_and_waiters = _futex.value().exchange(
        next_version, ::std::memory_order_release);
    if (current_version_and_waiters <= UINT16_MAX) {
        return;
    }
    _futex.wake_all();
...
void ConcurrentBoundedQueue<T, S>::SlotFutex::mark_tsan_acquire() noexcept {
#if ABSL_HAVE_THREAD_SANITIZER
...
template <typename T, typename S>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
void ConcurrentBoundedQueue<T, S>::SlotFutex::mark_tsan_release() noexcept {
#if ABSL_HAVE_THREAD_SANITIZER
...
void ConcurrentBoundedQueue<T, S>::
SlotFutex::block_until_reach_expected_version_slow(
        uint32_t current_version_and_waiters, uint16_t expected_version,
        const struct ::timespec* timeout,
        ::std::memory_order order) noexcept {
    int64_t begin_time_ns = timeout != nullptr ?
        ::absl::GetCurrentTimeNanos() : 0;
...
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentBoundedQueue<T, S>::
SlotFutex::spin_until_reach_expected_version_slow(
        uint32_t current_version_and_waiters, uint16_t expected_version,
        const struct ::timespec* timeout,
        ::std::memory_order order) noexcept {
    int64_t begin_time_ns = timeout != nullptr ?
        ::absl::GetCurrentTimeNanos() : 0;
    int64_t end_time_ns = begin_time_ns + (
        timeout != nullptr ?
            ::absl::ToInt64Nanoseconds(
                ::absl::DurationFromTimespec(*timeout)) : 0);
    while (true) {
        // 执行等待
        S::usleep(1000);
        // 读取新值
...
template <typename T, typename S>
inline ConcurrentBoundedQueue<T, S>::Iterator::Iterator(
        Slot* slot) noexcept :
    _slot(slot) {}
...
template <typename T, typename S>
inline typename ConcurrentBoundedQueue<T, S>::Iterator
ConcurrentBoundedQueue<T, S>::Iterator::operator+(ssize_t offset) const noexcept {
...
template <typename T, typename S>
inline T& ConcurrentBoundedQueue<T, S>::Iterator::operator*()
        const noexcept {
    return _slot->value;
}
...
template <typename T, typename S>
ConcurrentBoundedQueue<T, S>::SlotVector::SlotVector(SlotVector&& other) noexcept :
        SlotVector() {
    *this = ::std::move(other);
}
...
ConcurrentBoundedQueue<T, S>::SlotVector::operator=(SlotVector&& other) noexcept {
    swap(other);
    return *this;
}
...
template <typename T, typename S>
inline T& ConcurrentBoundedQueue<T, S>::SlotVector::value(size_t index) noexcept {
    return _slots[index].value;
}

template <typename T, typename S>
inline typename ConcurrentBoundedQueue<T, S>::Iterator
ConcurrentBoundedQueue<T, S>::SlotVector::value_iterator(size_t index) noexcept {
...
template <typename T, typename S>
inline void ConcurrentBoundedQueue<T, S>::SlotVector::swap(SlotVector& other) noexcept {
...
template <typename T, typename S>
ConcurrentBoundedQueue<T, S>::ConcurrentBoundedQueue(ConcurrentBoundedQueue&& other) noexcept :
        ConcurrentBoundedQueue() {
    swap(other);
}

template <typename T, typename S>
ConcurrentBoundedQueue<T, S>& ConcurrentBoundedQueue<T, S>::operator=(ConcurrentBoundedQueue&& other) noexcept {
    swap(other);
    return *this;
}

template <typename T, typename S>
ConcurrentBoundedQueue<T, S>::ConcurrentBoundedQueue(size_t min_capacity) noexcept :
        ConcurrentBoundedQueue() {
...
inline size_t ConcurrentBoundedQueue<T, S>::size() const noexcept {
    ...
    return next_push_index > next_pop_index ? next_push_index - next_pop_index : 0;
}
...
template <typename T, typename S>
template <typename U, typename ::std::enable_if<::std::is_assignable<T&, U>::value, int>::type>
...
template <bool CONCURRENT, bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE,
          typename U, typename ::std::enable_if<::std::is_assignable<T&, U>::value, int>::type>
inline void ConcurrentBoundedQueue<T, S>::push(U&& value) {
...
template <typename T, typename S>
template <bool CONCURRENT, bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, typename C, typename>
inline void ConcurrentBoundedQueue<T, S>::push(C&& callback) {
    // 自增取得序号
    auto index = CONCURRENT ? _next_push_index.fetch_add(1, ::std::memory_order_relaxed)
        : _next_push_index.load(::std::memory_order_relaxed);
    if (!CONCURRENT) {
        _next_push_index.store(index + 1, ::std::memory_order_relaxed);
    }
    // 处理单个数据
    deal<USE_FUTEX_WAIT, USE_FUTEX_WAKE, true>(::std::forward<C>(callback), index);
}
...
inline void ConcurrentBoundedQueue<T, S>::push_n(IT begin, IT end) {
    push_n<CONCURRENT, USE_FUTEX_WAIT, USE_FUTEX_WAKE>(
            [&] (Iterator dest_begin, Iterator dest_end)
...
template <bool CONCURRENT, bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, typename C, typename>
inline void ConcurrentBoundedQueue<T, S>::push_n(C&& callback, size_t num) {
    auto index = CONCURRENT ? _next_push_index.fetch_add(num, ::std::memory_order_relaxed)
        : _next_push_index.load(::std::memory_order_relaxed);
    if (!CONCURRENT) {
    ...
    if (index + num <= next_round_begin_index) {
        ...
        deal_n_continuously<USE_FUTEX_WAIT, USE_FUTEX_WAKE, true>(::std::forward<C>(callback),
                index, next_round_begin_index - index);
        deal_n_continuously<USE_FUTEX_WAIT, USE_FUTEX_WAKE, true>(::std::forward<C>(callback),
                next_round_begin_index, index + num - next_round_begin_index);
    }
}

template <typename T, typename S>
template <bool CONCURRENT, bool USE_FUTEX_WAKE, typename C, typename>
inline size_t ConcurrentBoundedQueue<T, S>::try_push_n(C&& callback, size_t num) {
...
inline void ConcurrentBoundedQueue<T, S>::push_n(C&& callback, RC&& reverse_callback, size_t num) {
...
inline void ConcurrentBoundedQueue<T, S>::pop(C&& callback) {
    // 自增取得序号
    auto index = CONCURRENT ? _next_pop_index.fetch_add(1, ::std::memory_order_relaxed)
        : _next_pop_index.load(::std::memory_order_relaxed);
    if (!CONCURRENT) {
        _next_pop_index.store(index + 1, ::std::memory_order_relaxed);
    }
    // 处理单个数据
    deal<USE_FUTEX_WAIT, USE_FUTEX_WAKE, false>(::std::forward<C>(callback), index);
}
...
inline bool ABSL_ATTRIBUTE_ALWAYS_INLINE
ConcurrentBoundedQueue<T, S>::try_pop(T& value) noexcept {
    return try_pop<CONCURRENT, USE_FUTEX_WAKE>(
        [&] (T& source) {
            value = ::std::move(source);
        }
    );
...
inline void ConcurrentBoundedQueue<T, S>::pop_n(IT begin, IT end) {
    pop_n<CONCURRENT, USE_FUTEX_WAIT, USE_FUTEX_WAKE>([&] (
            Iterator src_begin,
            Iterator src_end) ABSL_ATTRIBUTE_ALWAYS_INLINE {
...
template <bool CONCURRENT, bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, typename C, typename>
inline void ConcurrentBoundedQueue<T, S>::pop_n(C&& callback, size_t num) {
    auto index = CONCURRENT ? _next_pop_index.fetch_add(num, ::std::memory_order_relaxed)
        : _next_pop_index.load(::std::memory_order_relaxed);
    if (!CONCURRENT) {
    ...
    if (index + num <= next_round_begin_index) {
        ...
        deal_n_continuously<USE_FUTEX_WAIT, USE_FUTEX_WAKE, false>(::std::forward<C>(callback),
                index, next_round_begin_index - index);
        deal_n_continuously<USE_FUTEX_WAIT, USE_FUTEX_WAKE, false>(::std::forward<C>(callback),
                next_round_begin_index, index + num - next_round_begin_index);
    }
...
template <typename T, typename S>
template <bool CONCURRENT, bool USE_FUTEX_WAKE, typename C, typename>
inline size_t ConcurrentBoundedQueue<T, S>::try_pop_n(C&& callback, size_t num) {
...
template <typename T, typename S>
template <typename C, typename RC>
inline void ConcurrentBoundedQueue<T, S>::pop_n(C&& callback, RC&& reverse_callback, size_t num) {
...
template <typename T, typename S>
void ConcurrentBoundedQueue<T, S>::swap(ConcurrentBoundedQueue& other) noexcept {
    ::std::swap(_slots, other._slots);
...
template <typename T, typename S>
inline uint16_t ConcurrentBoundedQueue<T, S>::push_version_for_index(size_t index) noexcept {
    return (index >> _slot_bits) << 1;
}

template <typename T, typename S>
inline uint16_t ConcurrentBoundedQueue<T, S>::pop_version_for_index(size_t index) noexcept {
    return push_version_for_index(index) + 1;
}

template <typename T, typename S>
template <bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, bool PUSH_OR_POP, typename C>
inline void ConcurrentBoundedQueue<T, S>::deal(C&& callback, size_t index) noexcept {
    // 等待版本就绪
    auto expected_version = PUSH_OR_POP ? push_version_for_index(index) : pop_version_for_index(index);
    auto slot_index = index & _slot_mask;
    auto& futex = _slots.futex(slot_index);
...
inline bool ConcurrentBoundedQueue<T, S>::try_deal(C&& callback) noexcept {
    ...
    while (true) {
        ...
        if CONSTEXPR_SINCE_CXX17 (CONCURRENT) {
            // 竞争获取序号
            if (!next_index.compare_exchange_weak(
...
inline void ConcurrentBoundedQueue<T, S>::deal_n_continuously(
    ...
    for (size_t i = 0; i < num; ++i) {
        _slots.futex(slot_index + i)
            .template wait_until_reach_expected_version<
                USE_FUTEX_WAIT>(expected_version, nullptr, ::std::memory_order_relaxed);
    }
...
#endif // ABSL_HAVE_THREAD_SANITIZER
    // 批量执行回调
    callback(_slots.value_iterator(slot_index), _slots.value_iterator(slot_index + num));
    // 批量推进版本
...
#endif // ABSL_HAVE_THREAD_SANITIZER
    for (size_t i = 0; i < num; ++i) {
        _slots.futex(slot_index + i).set_version(expected_version + 1,
...
inline void ConcurrentBoundedQueue<T, S>::deal_n_continuously(C&& callback, RC&& reverse_callback,
        size_t index, size_t num) noexcept {
    // 等待整个区域版本就绪
    auto expected_version = PUSH_OR_POP ? push_version_for_index(index) : pop_version_for_index(index);
    auto slot_index = index & _slot_mask;
    for (size_t i = 0; i < num; ++i) {
        while (expected_version != _slots.futex(slot_index + i).version(::std::memory_order_relaxed)) {
            // 未全部就绪，检查是否队空/满，是则执行补偿
            auto need_index = PUSH_OR_POP ?
...
inline size_t ConcurrentBoundedQueue<T, S>::try_deal_n_continuously(
    ...
    auto& next_index = PUSH_OR_POP ? _next_push_index : _next_pop_index;
    if (CONCURRENT) {
        if (!next_index.compare_exchange_strong(index, index + num, ::std::memory_order_relaxed)) {
            return 0;
        }
    } else {
