...
class ConcurrentTransientTopic {
 ...
 public:
  ...
  inline ConcurrentTransientTopic(const ConcurrentTransientTopic&) noexcept =
      delete;
  ...
  template <bool CONCURRENT, typename U,
  ...
  class ConsumeRange {
   ...
   private:
    ...
    inline ConsumeRange(typename SlotVector::Snapshot snapshot, size_t begin,
 ...
 private:
  ...
  class SlotFutex {
   public:
    ...
    void wait_until_ready_slow(uint32_t current_status_and_waiters) noexcept;
