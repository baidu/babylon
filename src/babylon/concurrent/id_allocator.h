...
struct VersionedValue {
    ...
    typedef typename ::std::conditional<sizeof(T) == 2, uint32_t, uint64_t>::type VersionAndValue;
...
public:
    ...
    // 入参应理解为半闭半开区间[begin_id_value, end_id_value)，和典型的iterator体系相同
    // 例如当前已分配未释放的标识为0, 1, 3, 5，则可能会依次调用
    // callback(0, 2)
    // callback(3, 4)
    // callback(5, 6)
...
// 获取当前所在线程的唯一标识
// 原理上syscall __NR_gettid/pthread_self/std::this_thread::get_id都可以获取到唯一标识
...
public:
    ...
    // 对于传入callback的输入，应该理解为半闭半开区间[begin_id_value, end_id_value)
    // 例如当前活跃线程位0, 1, 3, 5，则可能会依次调用
    // callback(0, 2)
    // callback(3, 4)
    // callback(5, 6)
    template <typename T = void, typename C, typename = typename ::std::enable_if<IsInvocable<C, uint16_t, uint16_t>::value>::type>
