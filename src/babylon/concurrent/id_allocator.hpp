...
ABSL_ATTRIBUTE_NOINLINE VersionedValue<T> IdAllocator<T>::allocate() {
    ...
    while (current_head.value != FREE_LIST_TAIL) {
        ...
        new_head.value = _free_next_value[current_head.value].load(
                ::std::memory_order_relaxed);
        new_head.version = current_head.version;
        if (free_head().compare_exchange_weak(current_head, new_head,
                    ::std::memory_order_acq_rel)) {
            _free_next_value[current_head.value].store(
                    ACTIVE_FLAG, ::std::memory_order_relaxed);
            return current_head;
        }
    }
    current_head.version_and_value = next_value().fetch_add(
            1, ::std::memory_order_relaxed);
    _free_next_value.ensure(current_head.value).store(
            ACTIVE_FLAG, ::std::memory_order_relaxed);
    return current_head;
}
...
T IdAllocator<T>::end() const noexcept {
...
inline void IdAllocator<T>::for_each(C&& callback) const {
    ...
    snapshot.for_each(0, ::std::min<size_t>(
                snapshot.size(), next_value().load(::std::memory_order_acquire)),
...
template <typename T>
inline __attribute__((always_inline)) ::std::atomic<T>& IdAllocator<T>::next_value() noexcept {
    return reinterpret_cast<::std::atomic<T>&>(_next_value);
}

template <typename T>
inline __attribute__((always_inline)) const ::std::atomic<T>& IdAllocator<T>::next_value() const noexcept {
    return reinterpret_cast<const ::std::atomic<T>&>(_next_value);
}

template <typename T>
inline __attribute__((always_inline)) ::std::atomic<VersionedValue<T>>& IdAllocator<T>::free_head() noexcept {
...
inline ::std::basic_ostream<C, T>& operator<<(::std::basic_ostream<C, T>& os, VersionedValue<I> versioned_value) {
    return os << "VersionedValue[" << versioned_value.value << "@" << versioned_value.version << "]";
}
...
ThreadId::current_thread_id() noexcept {
    thread_local ThreadId id(
            internal::concurrent_id_allocator::
                IdAllocatorFotType<T>::instance());
    return id._value;
}

template <typename T>
inline __attribute__((always_inline)) uint16_t ThreadId::end() noexcept {
    return internal::concurrent_id_allocator::
        IdAllocatorFotType<T>::instance().end();
}

template <typename T, typename C, typename>
inline __attribute__((always_inline)) void ThreadId::for_each(C&& callback) {
    internal::concurrent_id_allocator::
        IdAllocatorFotType<T>::instance().for_each(::std::forward<C>(callback));
}

inline ThreadId::ThreadId(IdAllocator<uint16_t>& allocator) : _allocator(allocator), _value(allocator.allocate()) {}
