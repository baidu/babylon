...
inline int SchedInterface::futex_wait(
    uint32_t* futex, uint32_t val, const struct ::timespec* timeout) noexcept {
  return ::syscall(__NR_futex, futex, (FUTEX_WAIT | FUTEX_PRIVATE_FLAG), val,
                   timeout);
}
...
inline int SchedInterface::futex_wake_all(uint32_t* futex) noexcept {
  return ::syscall(__NR_futex, futex, (FUTEX_WAKE | FUTEX_PRIVATE_FLAG),
                   INT32_MAX);
}
...
template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline Futex<
    S, typename ::std::enable_if<!S::futex_need_create()>::type>::
    Futex(uint32_t value) noexcept
    : _value(value) {}

template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline ::std::atomic<uint32_t>&
Futex<S, typename ::std::enable_if<!S::futex_need_create()>::type>::
    value() noexcept {
  return reinterpret_cast<::std::atomic<uint32_t>&>(_value);
}

template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline const ::std::atomic<uint32_t>&
Futex<S, typename ::std::enable_if<!S::futex_need_create()>::type>::value()
    const noexcept {
  return reinterpret_cast<const ::std::atomic<uint32_t>&>(_value);
}

template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline int
Futex<S, typename ::std::enable_if<!S::futex_need_create()>::type>::wait(
...
ABSL_ATTRIBUTE_ALWAYS_INLINE inline Futex<
    S,
    typename ::std::enable_if<S::futex_need_create()>::type>::Futex() noexcept
    : _value(S::create_futex()) {}

template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline Futex<
    S, typename ::std::enable_if<S::futex_need_create()>::type>::
    Futex(Futex&& other) noexcept
    : Futex() {
...
ABSL_ATTRIBUTE_ALWAYS_INLINE inline Futex<
    S, typename ::std::enable_if<S::futex_need_create()>::type>::
    Futex(const Futex& other) noexcept
    : Futex() {
  *_value = *other._value;
}
...
template <typename S>
ABSL_ATTRIBUTE_ALWAYS_INLINE inline ::std::atomic<uint32_t>& Futex<
    S,
    typename ::std::enable_if<S::futex_need_create()>::type>::value() noexcept {
  return *reinterpret_cast<::std::atomic<uint32_t>*>(_value);
}
...
template <typename S>
inline int Futex<S, typename ::std::enable_if<S::futex_need_create()>::type>::
    wake_one() noexcept {
  return S::futex_wake_one(_value);
}
