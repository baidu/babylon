...
struct PairKeyExtractor {
    ...
    template <typename T, typename = typename ::std::enable_if<
        !::std::is_convertible<T, const ::std::pair<const K, V>&>::value>::type>
...
public:
    ...
    class GroupIndex {
    public:
        inline GroupIndex(size_t index) noexcept :
            _index {index} {}
    ...
    inline explicit GroupIterator(uint64_t mask) noexcept :
        _mask {mask} {}
    ...
    GroupIndex operator++(int) noexcept {
#if BABYLON_TMP_GROUP_USE_NEON
...
public:
    ...
    inline ABSL_ATTRIBUTE_ALWAYS_INLINE
    Group(::std::atomic<int8_t>* controls) noexcept
    ...
    inline ABSL_ATTRIBUTE_ALWAYS_INLINE
    GroupIterator match(int8_t check) noexcept {
#if BABYLON_TMP_GROUP_USE_SSE
        uint16_t mask = _mm_movemask_epi8(
            _mm_cmpeq_epi8(_controls, _mm_set1_epi8(check)));
#elif BABYLON_TMP_GROUP_USE_NEON
        uint16x8_t mask128 = vreinterpretq_u16_u8(
            vceqq_s8(_controls, vdupq_n_s8(check)));
    ...
    inline ABSL_ATTRIBUTE_ALWAYS_INLINE
    GroupIterator match_empty() noexcept {
#if BABYLON_TMP_GROUP_USE_SSE
        uint16_t mask = _mm_movemask_epi8(_controls);
#elif BABYLON_TMP_GROUP_USE_NEON
        uint16x8_t mask128 = vreinterpretq_u16_u8(
            vceqq_s8(
                vandq_s8(_controls, vdupq_n_s8(0x80)),
                vdupq_n_s8(0x80)
            )
        );
        uint8x8_t mask64 = vshrn_n_u16(mask128, 4);
    ...
    inline ABSL_ATTRIBUTE_ALWAYS_INLINE
    GroupIterator match_non_empty() noexcept {
#if BABYLON_TMP_GROUP_USE_SSE
        uint16_t mask = ~_mm_movemask_epi8(_controls);
#elif BABYLON_TMP_GROUP_USE_NEON
        uint16x8_t mask128 = vreinterpretq_u16_u8(
...
public:
    ...
    template <bool OTHER_CONST,
              typename = typename ::std::enable_if<CONST ||
                                                   !OTHER_CONST>::type>
...
private:
    ...
    inline Iterator(const ConcurrentFixedSwissTable& table,
                    size_t index, GroupIterator) noexcept;
...
private:
    inline Iterator(TableNode* next,
                    TableIterator iter) noexcept;
...
template <bool CONST>
inline typename
ConcurrentFixedSwissTable<T, H, E>::template Iterator<CONST>::reference
ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::operator*() const noexcept {
...
template <bool CONST>
inline typename
ConcurrentFixedSwissTable<T, H, E>::template Iterator<CONST>::pointer
ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::operator->() const noexcept {
    return &_table->at(_index);
}
...
ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::operator++() noexcept {
    ...
    if (!_iter) {
        auto base_index_and_iter = _table->find_first_non_empty(
            base_index + Group::SIZE);
...
template <typename T, typename H, typename E>
template <bool CONST>
inline ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::
...
template <bool CONST>
inline ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::Iterator(
    const ConcurrentFixedSwissTable& table,
    size_t index) noexcept :
...
inline ConcurrentFixedSwissTable<T, H, E>::Iterator<CONST>::Iterator(
    const ConcurrentFixedSwissTable& table,
    size_t index, GroupIterator iter) noexcept :
...
ConcurrentFixedSwissTable<T, H, E>::ConcurrentFixedSwissTable() noexcept :
    _controls {Group::s_dummy_controls},
    _bucket_mask {Group::GROUP_MASK} {}
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
ConcurrentFixedSwissTable<T, H, E>&
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
typename ConcurrentFixedSwissTable<T, H, E>::iterator
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentFixedSwissTable<T, H, E>::clear() noexcept {
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentFixedSwissTable<T, H, E>::swap(
        ConcurrentFixedSwissTable& other) noexcept {
...
template <typename T, typename H, typename E>
inline size_t
ConcurrentFixedSwissTable<T, H, E>::bucket_count() const noexcept {
    return _bucket_mask + 1;
}

template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentFixedSwissTable<T, H, E>::rehash(
        size_t min_bucket_count) noexcept {
    if (ABSL_PREDICT_FALSE(_controls == Group::s_dummy_controls)) {
        construct_with_bucket(min_bucket_count);
        return;
    ...
    construct_with_bucket(::std::max<size_t>(new_bucket_count, saved_table.size()));
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentFixedSwissTable<T, H, E>::reserve(size_t min_size) noexcept {
    if (ABSL_PREDICT_FALSE(_controls == Group::s_dummy_controls)) {
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentFixedSwissTable<T, H, E>::construct_with_bucket(
        size_t min_bucket_count) noexcept {
    auto bucket_count = ::absl::bit_ceil(
        ::std::max<size_t>(min_bucket_count, Group::SIZE));
    auto allocate_size = calculate_allocate_size(bucket_count);
    auto values_offset = calculate_values_offset(bucket_count);
    
    auto buffer = static_cast<int8_t*>(
        ::operator new(allocate_size, VALUE_ALIGNMENT));
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
size_t ConcurrentFixedSwissTable<T, H, E>::calculate_allocate_size(
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
size_t ConcurrentFixedSwissTable<T, H, E>::calculate_values_offset(
        size_t bucket_count) noexcept {
...
ABSL_ATTRIBUTE_NOINLINE
::std::pair<typename ConcurrentFixedSwissTable<T, H, E>::iterator, bool>
    ...
    while (step <= _bucket_mask) {
        ...
        if (iter) {
            ...
            if (control.compare_exchange_strong(
                        control_value, Group::BUSY_CONTROL,
                        ::std::memory_order_acquire,
                        ::std::memory_order_relaxed)) {
                UsesAllocatorConstructor::construct(
                    &at(index), allocator_type(),
...
template <bool CONST>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
typename ConcurrentTransientHashSet<T, H, E>::
    template Iterator<CONST>::reference
ConcurrentTransientHashSet<T, H, E>::Iterator<CONST>::
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
typename ConcurrentTransientHashSet<T, H, E>::
    template Iterator<CONST>::pointer
ConcurrentTransientHashSet<T, H, E>::Iterator<CONST>::
        operator->() const noexcept {
    return &**this;
}
...
template <bool CONST>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
bool ConcurrentTransientHashSet<T, H, E>::Iterator<CONST>::operator==(
        Iterator other) const noexcept {
...
ConcurrentTransientHashSet<T, H, E>::Iterator<CONST>::operator++() noexcept {
    ++_iter;
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
ConcurrentTransientHashSet<T, H, E>&
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
ConcurrentTransientHashSet<T, H, E>::~ConcurrentTransientHashSet() noexcept {
...
template <typename T, typename H, typename E>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
bool ConcurrentTransientHashSet<T, H, E>::empty() const noexcept {
    return _head.table.empty();
}

template <typename T, typename H, typename E>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
size_t ConcurrentTransientHashSet<T, H, E>::size() const noexcept {
    auto node = _head.next.load(::std::memory_order_acquire);
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentTransientHashSet<T, H, E>::clear() noexcept {
    auto node = _head.next.load(::std::memory_order_relaxed);
    if (ABSL_PREDICT_TRUE(node == nullptr)) {
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentTransientHashSet<T, H, E>::swap(
        ConcurrentTransientHashSet& other) noexcept {
...
template <typename K>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
size_t ConcurrentTransientHashSet<T, H, E>::count(
        const K& key) const noexcept {
    return contains(key) ? 1 : 0;
}
...
template <typename T, typename H, typename E>
inline size_t ConcurrentTransientHashSet<T, H, E>::bucket_count() const noexcept {
    return _head.table.bucket_count();
}

template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentTransientHashSet<T, H, E>::rehash(
        size_t min_bucket_count) noexcept {
    auto node = _head.next.load(::std::memory_order_relaxed);
    if (ABSL_PREDICT_TRUE(node == nullptr)) {
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentTransientHashSet<T, H, E>::reserve(size_t min_size) noexcept {
    auto node = _head.next.load(::std::memory_order_relaxed);
    if (ABSL_PREDICT_TRUE(node == nullptr)) {
...
template <typename T, typename H, typename E>
ABSL_ATTRIBUTE_NOINLINE
size_t ConcurrentTransientHashSet<T, H, E>::total_size(
...
ConcurrentTransientHashMap<K, V, H>::try_emplace(KK&& key,
                                                 Args&&... args) noexcept {
    return this->emplace(::std::forward<KK>(key),
                         ::std::forward<Args>(args)...);
}

template <typename K, typename V, typename H>
template <typename KK>
inline V& ConcurrentTransientHashMap<K, V, H>::operator[](
        KK&& key) noexcept {
