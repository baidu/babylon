...
public:
    ...
    inline EnumerableThreadLocal& operator=(EnumerableThreadLocal&&) noexcept = default;
    ...
    template <typename C, typename = typename ::std::enable_if<IsInvocable<C, T*, T*>::value>::type>
    inline void for_each(C&& callback) {
        auto snapshot = _storage.snapshot();
        snapshot.for_each(0, ::std::min(ThreadId::end<T>(), static_cast<uint16_t>(snapshot.size())), callback);
    }

    template <typename C, typename = typename ::std::enable_if<IsInvocable<C, const T*, const T*>::value>::type>
...
public:
    ...
    CompactEnumerableThreadLocal& operator=(
            CompactEnumerableThreadLocal&& other) noexcept;
    CompactEnumerableThreadLocal& operator=(
            const CompactEnumerableThreadLocal&) = delete;
    ~CompactEnumerableThreadLocal() noexcept;
    ...
    // 遍历所有【当前或曾经存在】的线程局部存储
    template <typename C, typename = typename ::std::enable_if<IsInvocable<C, T&>::value>::type>
    inline void for_each(C&& callback) {
    ...
    template <typename C, typename = typename ::std::enable_if<IsInvocable<C, const T&>::value>::type>
    inline void for_each(C&& callback) const {
...
private:
    ...
    static_assert(NUM_PER_CACHELINE > 1, "sizeof(T) to large to fit into cache line");
...
template <typename T>
template <typename C>
inline EnumerableThreadLocal<T>::EnumerableThreadLocal(
        C&& constructor) noexcept : _storage {::std::forward<C>(constructor)} {
    // 确保内部使用的ThreadId此时初始化，建立正确的析构顺序
    ThreadId::end<T>();
}
