...
inline ConcurrentTransientTopic<T, S>::ConcurrentTransientTopic(
        ConcurrentTransientTopic&& other) noexcept : ConcurrentTransientTopic {} {
    *this = ::std::move(other);
}
...
inline void ConcurrentTransientTopic<T, S>::publish_n(
        size_t num, C&& callback) noexcept {
...
template <typename T, typename S>
inline void
ConcurrentTransientTopic<T, S>::SlotFutex::set_closed() noexcept {
    auto& value = _futex.value();
    auto& status = reinterpret_cast<::std::atomic<uint16_t>&>(value);
...
void ConcurrentTransientTopic<T, S>::SlotFutex::wakeup_waiters_slow(
...
template <typename T, typename S>
inline bool
ConcurrentTransientTopic<T, S>::SlotFutex::is_published() const noexcept {
    auto& value = _futex.value();
...
template <typename T, typename S>
inline bool
ConcurrentTransientTopic<T, S>::SlotFutex::is_closed() const noexcept {
    auto& value = _futex.value();
    auto& status = reinterpret_cast<const ::std::atomic<uint16_t>&>(value);
...
template <typename T, typename S>
ABSL_ATTRIBUTE_NOINLINE
void ConcurrentTransientTopic<T, S>::SlotFutex::wait_until_ready_slow(
        uint32_t current_status_and_waiters) noexcept {
    uint16_t status = current_status_and_waiters;
...
void ConcurrentTransientTopic<T, S>::SlotFutex::mark_tsan_acquire() noexcept {
#if ABSL_HAVE_THREAD_SANITIZER
...
template <typename T, typename S>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE
void ConcurrentTransientTopic<T, S>::SlotFutex::mark_tsan_release() noexcept {
#if ABSL_HAVE_THREAD_SANITIZER
...
template <typename T, typename S>
inline typename ConcurrentTransientTopic<T, S>::Iterator
ConcurrentTransientTopic<T, S>::Iterator::operator+(ssize_t offset) const noexcept {
...
inline ConcurrentTransientTopic<T, S>::ConsumeRange::ConsumeRange(
    typename SlotVector::Snapshot snapshot,
    size_t begin, size_t size) noexcept :
        _snapshot(snapshot), _begin(begin), _size(size) {}

template <typename T, typename S>
inline size_t
ConcurrentTransientTopic<T, S>::ConsumeRange::size() const noexcept {
    return _size;
}
...
inline ConcurrentTransientTopic<T, S>::ConstConsumeRange::ConstConsumeRange(
    ConsumeRange other) noexcept : ConsumeRange(other) {}
...
inline ConcurrentTransientTopic<T, S>::ConstConsumer::ConstConsumer(
    const Consumer& other) noexcept : Consumer {other} {}
