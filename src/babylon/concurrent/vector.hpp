...
template <typename T, typename D>
inline RetireList<T, D>& RetireList<T, D>::operator=(RetireList&& other) noexcept {
...
void RetireList<T, D>::retire(T* data) {
    ...
    if (expire(head, timestamp)) {
        node->next = nullptr;
        if (_head.compare_exchange_strong(head, new_head, ::std::memory_order_acq_rel)) {
            delete_list(head);
            return;
        }
    }
    ...
    } while (!_head.compare_exchange_weak(head, new_head, ::std::memory_order_acq_rel));
}
...
template <typename T, typename D>
inline __attribute__((always_inline)) uint16_t RetireList<T, D>::get_timestamp(uint64_t head) noexcept {
    return head >> 48;
}

template <typename T, typename D>
inline __attribute__((always_inline)) uint64_t RetireList<T, D>::make_head(Node* node, uint64_t timestamp) noexcept {
    return (timestamp << 48) | reinterpret_cast<uint64_t>(node);
}

template <typename T, typename D>
inline __attribute__((always_inline)) uint16_t RetireList<T, D>::get_current_timestamp() noexcept {
    ::timespec spec;
...
inline __attribute__((always_inline)) T& ConcurrentVector<T, BLOCK_SIZE>::Snapshot::operator[](size_t index) const noexcept {
    return _block_table->blocks[_meta.block_index(index)][_meta.block_offset(index)];
}

template <typename T, size_t BLOCK_SIZE>
template <typename IT, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::Snapshot::copy_n(IT begin, size_t size, size_t offset) {
...
template <typename T, size_t BLOCK_SIZE>
inline void ConcurrentVector<T, BLOCK_SIZE>::Snapshot::fill_n(size_t offset, size_t size, const T& value) {
    for_each(offset, offset + size, [&] (T* iter, T* end) {
...
inline void ConcurrentVector<T, BLOCK_SIZE>::Snapshot::for_each(size_t begin, size_t end, C&& callback) {
...
template <typename T, size_t BLOCK_SIZE>
template <typename C, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::Snapshot::for_each(size_t begin, size_t end, C&& callback) const {
    auto block_index = _meta.block_index(begin);
    auto block_offset = _meta.block_offset(begin);
    auto end_block_index = _meta.block_index(end);
    auto end_block_offset = _meta.block_offset(end);
...
template <typename T, size_t BLOCK_SIZE>
inline ConcurrentVector<T, BLOCK_SIZE>::Snapshot::Snapshot(Meta meta, BlockTable* block_table) noexcept :
...
template <typename T, size_t BLOCK_SIZE>
inline const T& ConcurrentVector<T, BLOCK_SIZE>::ConstSnapshot::operator[] (size_t index) const noexcept {
    return _snapshot[index];
}

template <typename T, size_t BLOCK_SIZE>
template <typename C, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::ConstSnapshot::for_each(size_t begin, size_t end, C&& callback) const {
    _snapshot.for_each(begin, end, ::std::forward<C>(callback));
}

template <typename T, size_t BLOCK_SIZE>
inline size_t ConcurrentVector<T, BLOCK_SIZE>::ConstSnapshot::size() const noexcept {
    return _snapshot.size();
}

template <typename T, size_t BLOCK_SIZE>
inline ConcurrentVector<T, BLOCK_SIZE>::ConstSnapshot::ConstSnapshot(Meta meta, BlockTable* block_table) noexcept :
...
template <typename T, size_t BLOCK_SIZE>
inline __attribute__((always_inline)) void ConcurrentVector<T, BLOCK_SIZE>::BlockTableDeleter::operator()(
...
template <typename T, size_t BLOCK_SIZE>
inline size_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::set_block_size(size_t block_size_hint) noexcept {
...
template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::block_size() const noexcept {
    return _block_mask + 1;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::block_mask() const noexcept {
    return _block_mask;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::block_mask_bits() const noexcept {
    return _block_mask_bits;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::block_index(size_t index) const noexcept {
    return index >> _block_mask_bits;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::DynamicMeta::block_offset(size_t index) const noexcept {
...
template <typename T, size_t BLOCK_SIZE>
inline size_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::set_block_size(size_t) noexcept {
    return BLOCK_SIZE;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::block_size() const noexcept {
    return BLOCK_SIZE;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::block_mask() const noexcept {
    return BLOCK_SIZE - 1;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::block_mask_bits() const noexcept {
    return BLOCK_MASK_BITS;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::block_index(size_t index) const noexcept {
    return index >> BLOCK_MASK_BITS;
}

template <typename T, size_t BLOCK_SIZE>
inline uint32_t ConcurrentVector<T, BLOCK_SIZE>::StaticMeta::block_offset(size_t index) const noexcept {
...
inline ConcurrentVector<T, BLOCK_SIZE>::ConcurrentVector(
        size_t block_size_hint) noexcept :
            ConcurrentVector {block_size_hint,
...
inline ConcurrentVector<T, BLOCK_SIZE>::ConcurrentVector(
    ...
    _meta.set_block_size(block_size_hint);
    _block_table.store(const_cast<BlockTable*>(&EMPTY_BLOCK_TABLE), ::std::memory_order_relaxed);
}
...
template <typename T, size_t BLOCK_SIZE>
inline const T& ConcurrentVector<T, BLOCK_SIZE>::operator[](size_t index) const noexcept {
    return snapshot()[index];
}

template <typename T, size_t BLOCK_SIZE>
inline void ConcurrentVector<T, BLOCK_SIZE>::fill_n(size_t offset, size_t size, const T& value) {
    reserved_snapshot(offset + size).fill_n(offset, size, value);
}

template <typename T, size_t BLOCK_SIZE>
template <typename IT, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::copy_n(IT begin, size_t size, size_t offset) {
    reserved_snapshot(offset + size).copy_n(begin, size, offset);
}

template <typename T, size_t BLOCK_SIZE>
template <typename C, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::for_each(size_t begin, size_t end, C&& callback) {
    reserved_snapshot(end).for_each(begin, end, ::std::forward<C>(callback));
}

template <typename T, size_t BLOCK_SIZE>
template <typename C, typename>
inline void ConcurrentVector<T, BLOCK_SIZE>::for_each(size_t begin, size_t end, C&& callback) const {
    snapshot().for_each(begin, end, ::std::forward<C>(callback));
}

template <typename T, size_t BLOCK_SIZE>
inline typename ConcurrentVector<T, BLOCK_SIZE>::Snapshot ConcurrentVector<T, BLOCK_SIZE>::snapshot() noexcept {
    return Snapshot(_meta, _block_table.load(::std::memory_order_acquire));
}

template <typename T, size_t BLOCK_SIZE>
inline typename ConcurrentVector<T, BLOCK_SIZE>::ConstSnapshot ConcurrentVector<T, BLOCK_SIZE>::snapshot() const noexcept {
    return ConstSnapshot(_meta, _block_table.load(::std::memory_order_acquire));
}

template <typename T, size_t BLOCK_SIZE>
inline typename ConcurrentVector<T, BLOCK_SIZE>::Snapshot ConcurrentVector<T, BLOCK_SIZE>::reserved_snapshot(size_t size) {
    auto* block_table = get_qualified_block_table(_meta.block_index(size + _meta.block_mask()));
...
void ConcurrentVector<T, BLOCK_SIZE>::swap(ConcurrentVector& other) noexcept {
    ...
    ::std::swap(_constructor, other._constructor);
    {
        ...
        _block_table.store(other._block_table.load(
...
ConcurrentVector<T, BLOCK_SIZE>::create_block_table(size_t num) noexcept {
    ...
    auto* block_table = reinterpret_cast<BlockTable*>(
        ::operator new(allocation_size, ::std::align_val_t(BABYLON_CACHELINE_SIZE)));
    block_table->size = num;
    return block_table;
}

template <typename T, size_t BLOCK_SIZE>
inline void ConcurrentVector<T, BLOCK_SIZE>::delete_block_table(BlockTable* block_table) noexcept {
    if (block_table != &EMPTY_BLOCK_TABLE) {
        auto allocation_size = calculate_block_table_allocation_size(block_table->size);
...
inline T* ConcurrentVector<T, BLOCK_SIZE>::create_block() {
    ...
    auto* block = reinterpret_cast<T*>(
        ::operator new(allocation_size, block_alignment()));
    if (_constructor) {
...
ConcurrentVector<T, BLOCK_SIZE>::block_alignment() noexcept {
    return ::std::align_val_t(::std::max<size_t>(BABYLON_CACHELINE_SIZE, alignof(T)));
}
...
template <typename T, size_t BLOCK_SIZE>
inline typename ConcurrentVector<T, BLOCK_SIZE>::BlockTable*
ConcurrentVector<T, BLOCK_SIZE>::get_qualified_block_table(size_t expect_block_num) {
...
typename ConcurrentVector<T, BLOCK_SIZE>::BlockTable*
ConcurrentVector<T, BLOCK_SIZE>::get_qualified_block_table_slow(BlockTable* block_table, size_t expect_block_num) {
    auto block_num = block_table->size;
    auto* new_block_table = create_block_table(expect_block_num);
    while (true) {
        __builtin_memcpy(new_block_table->blocks, block_table->blocks, block_num * sizeof(char*));
...
template <typename T, size_t BLOCK_SIZE>
constexpr typename ConcurrentVector<T, BLOCK_SIZE>::BlockTable ConcurrentVector<T, BLOCK_SIZE>::EMPTY_BLOCK_TABLE;
