...
ObjectPool<T>::pop() noexcept {
    ...
    if (_object_creator) {
        using IT = typename ConcurrentBoundedQueue<
            ::std::unique_ptr<T>>::Iterator;
        _free_objects.template pop_n([&] (IT iter, IT) {
...
void ObjectPool<T>::push(::std::unique_ptr<T>&& object) noexcept {
    ...
    if (_object_creator) {
        ...
        _free_objects.template push_n([&] (IT iter, IT) {
...
template <typename T>
inline typename ObjectPool<T>::Deleter&
ObjectPool<T>::Deleter::operator=(Deleter&& other) noexcept {
    ::std::swap(_pool, other._pool);
}
