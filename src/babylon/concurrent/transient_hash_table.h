...
template <typename T, typename H = ::std::hash<T>,
          typename E =
              internal::concurrent_transient_hash_table::IdentityKeyExtractor>
class ConcurrentFixedSwissTable {
 private:
  template <typename K>
  using IsHashableAsKey =
      IsHashable<H, decltype(E::extract(::std::declval<K>()))>;

  template <typename K>
  using IsComparableAsKey =
      IsEqualityComparable<decltype(E::extract(::std::declval<T>())),
 ...
 public:
  ...
  struct CanUseAsKey {
    static constexpr bool value =
        IsHashableAsKey<K>::value && IsComparableAsKey<K>::value;
  };
  ...
  template <typename K, typename... Args,
            typename = typename ::std::enable_if<CanUseAsKey<K>::value>::type>
 ...
 private:
  ...
  static constexpr ::std::align_val_t VALUE_ALIGNMENT =
      static_cast<::std::align_val_t>(alignof(CachelineAligned<T>));
  ...
  template <typename K, typename... Args>
  ::std::pair<iterator, bool> do_emplace(K&& key_or_value,
                                         Args&&... args) noexcept;
...
class ConcurrentTransientHashSet {
 ...
 public:
  ...
  ConcurrentTransientHashSet& operator=(ConcurrentTransientHashSet&&) noexcept;
...
ConcurrentTransientHashSet<T, H, E>::TableNode::TableNode(
    ...
    : table {min_bucket_count} {}

template <typename T, typename H, typename E>
inline ABSL_ATTRIBUTE_ALWAYS_INLINE void
ConcurrentTransientHashSet<T, H, E>::TableNode::swap(
    TableNode& other) noexcept {
...
class ConcurrentTransientHashMap
 ...
 public:
  ...
  inline ::std::pair<iterator, bool> try_emplace(KK&& key,
                                                 Args&&... args) noexcept;
