...
class ConcurrentBoundedQueue {
 private:
  ...
  class SlotFutex {
   public:
    ...
    // 推进版本号 && 唤醒等待者
    inline void set_version_and_wakeup_waiters(uint16_t next_version) noexcept;
    // 重置到初始状态
 ...
 public:
  ...
  // 单元素入队
  // 默认版本push = push<CONCURRENT = true, USE_FUTEX_WAIT = true,
  // USE_FUTEX_WAKE = true> C为回调函数，在获得入队空位后会按照C(T&
  // value)调用，C可以无竞争操作value，在C返回后value被发布 CONCURRENT:
  // 是否有其他push可能并发，无并发时底层序号推进将采用非原子操作完成，提升性能
  ...
  inline void push(U&& value);
  template <
      bool CONCURRENT, bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, typename C,
      typename = typename ::std::enable_if<IsInvocable<C, T&>::value>::type>
  inline void push(C&& callback);
  ...
  inline bool try_push(U&& value) noexcept;
  template <
      bool CONCURRENT, bool USE_FUTEX_WAKE, typename C,
      typename = typename ::std::enable_if<IsInvocable<C, T&>::value>::type>
  inline bool try_push(C&& callback) noexcept;

  // 批量入队
  // 默认版本push_n = push_n<CONCURRENT = true, USE_FUTEX_WAIT = true,
  // USE_FUTEX_WAKE = true> C为回调函数，在获得入队空位后会按照C(iter begin,
  // iter end)调用，区间为[begin, end)语义，C可以无竞争操作区间
  ...
  template <bool CONCURRENT, bool USE_FUTEX_WAKE, typename C,
            typename = typename ::std::enable_if<
                IsInvocable<C, Iterator, Iterator>::value>::type>
  ...
  // 单元素出队
  // 默认版本pop = pop<CONCURRENT = true, USE_FUTEX_WAIT = true, USE_FUTEX_WAKE
  // = true> C为回调函数，在获得出队元素后会按照C(T&
  // value)调用，C可以无竞争操作value
  // 比如典型可以拷贝或者移动到所需空间中在C返回后value被消费，不再继续使用
  // CONCURRENT:
  // 是否有其他pop可能并发，无并发时底层序号推进将采用非原子操作完成，提升性能
  // USE_FUTEX_WAIT: 是否使用futex_wait阻塞等待，否则采用自旋等待
  ...
  // 批量出队
  // 默认版本pop_n = pop_n<CONCURRENT = true, USE_FUTEX_WAIT = true,
  // USE_FUTEX_WAKE = true> C为回调函数，在获得出队元素后会按照C(iter begin,
  // iter end)调用，区间为[begin, end)语义，C可以无竞争操作区间
  ...
  // return: 实际出队元素的数目，可能小于num
  template <bool USE_FUTEX_WAKE, typename C,
            typename = typename ::std::enable_if<
                IsInvocable<C, Iterator, Iterator>::value>::type>
 ...
 private:
  ...
  template <bool USE_FUTEX_WAIT, bool USE_FUTEX_WAKE, bool PUSH_OR_POP,
            typename C>
  inline void deal_n_continuously(C&& callback, size_t index,
                                  size_t num) noexcept;
  ...
  template <bool CONCURRENT, bool USE_FUTEX_WAKE, bool PUSH_OR_POP, typename C>
  inline size_t try_deal_n_continuously(C&& callback, size_t index,
                                        size_t num) noexcept;
  ...
  alignas(BABYLON_CACHELINE_SIZE)::std::atomic<size_t> _next_push_index =
      ATOMIC_VAR_INIT(0);
  alignas(BABYLON_CACHELINE_SIZE)::std::atomic<size_t> _next_pop_index =
      ATOMIC_VAR_INIT(0);
