...
public:
    ...
    template <typename A>
    inline BasicStringView(const ::std::basic_string<C, T, A>& other) noexcept : Base(other.data(), other.size()) {}
};
#else // __cplusplus < 201703L
...
public:
    ...
    inline constexpr BasicStringView(const BasicStringView& other) noexcept = default;
    inline CONSTEXPR_SINCE_CXX14 BasicStringView& operator=(BasicStringView&& other) noexcept = default;
    inline CONSTEXPR_SINCE_CXX14 BasicStringView& operator=(const BasicStringView& other) noexcept = default;

    inline constexpr BasicStringView(const_pointer c_str) noexcept;
    inline constexpr BasicStringView(const_pointer buffer, size_type size) noexcept;
    template <typename A>
    ...
    inline constexpr const_reference operator[](size_type position) const noexcept;
    ...
    inline size_type copy(pointer dest, size_type count, size_type position = 0) const;
    inline constexpr BasicStringView substr(size_type position = 0, size_type count = npos) const noexcept;

    inline BABYLON_CONSTEXPR int32_t compare(const BasicStringView& other) const noexcept;
    inline BABYLON_CONSTEXPR int32_t compare(size_type position, size_type count,
            const BasicStringView& other) const noexcept;
    inline BABYLON_CONSTEXPR int32_t compare(size_type position, size_type count,
            const BasicStringView& other, size_type other_position,
            size_type other_count) const noexcept;
    inline BABYLON_CONSTEXPR int32_t compare(size_type position, size_type count, const_pointer c_str) const noexcept;
    inline BABYLON_CONSTEXPR int32_t compare(size_type position, size_type count,
    ...
    // 不过有个缺陷是因为无法直接转换为std::string，因此对于copy initialization的场景
    // 需要调整使用方法
    // void function(const ::std::string&)
    // std::string s = sv;    ->    std::string s(sv);  // direct initialization
    // function(sv);          ->    function({sv});     // copy list initialization
    inline constexpr operator ::std::initializer_list<value_type>() const noexcept;
