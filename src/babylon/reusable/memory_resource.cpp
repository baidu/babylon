...
size_t
ExclusiveMonotonicBufferResource::allocate_oversize_page_num() noexcept {
...
void* ExclusiveMonotonicBufferResource::do_allocate(
...
void* ExclusiveMonotonicBufferResource::do_allocate_with_page_in_new_page_array(
    ...
    if (_last_page_allocated + sizeof(PageArray) <= page_size) {
        ...
        auto new_page_array = reinterpret_cast<PageArray*>(
                _last_page + _last_page_allocated);
        ...
        auto additional_page = reinterpret_cast<char*>(
                _page_allocator->allocate());
        _space_allocated += page_size;
...
void* ExclusiveMonotonicBufferResource::do_allocate_in_oversize_page(
    ...
    if (_last_oversize_page_pointer > _last_oversize_page_array->pages) {
        auto page = reinterpret_cast<char*>(
                _upstream->allocate(bytes, alignment));
    ...
    _last_oversize_page_pointer = &new_page_array->pages[
        DESTROY_TASK_ARRAY_CAPACITY - 1];
    _last_oversize_page_pointer->page = page;
...
void ExclusiveMonotonicBufferResource::release() noexcept {
    ...
    while (_last_page_array != nullptr) {
        ...
        auto size = _last_page_array->pages + PAGE_ARRAY_CAPACITY -
            _last_page_pointer;
        ...
        for (auto i = 0; i < size; ++i) {
            tmp_pages[i] = SanitizerHelper::unpoison(
                    _last_page_pointer[i],
                    _page_allocator->page_size());
        }
        _page_allocator->deallocate(
                reinterpret_cast<void**>(tmp_pages), size);
...
void* SharedMonotonicBufferResource::do_allocate(
        size_t bytes, size_t alignment) noexcept {
...
void SharedMonotonicBufferResource::register_thread_constructor() noexcept {
    ...
    _resources.set_constructor([&] (
            ExclusiveMonotonicBufferResource* ptr) {
...
void SwissMemoryResource::set_page_allocator(PageAllocator& page_allocator) noexcept {
...
void SwissMemoryResource::set_upstream(::std::pmr::memory_resource& upstream) noexcept {
    // 高版本protobuf，比如3.17.x，在Reset时就会使用alloc来分配一些初始空间
    // 采用析构和再构造来避免这个空间持有，让底层内存可以完全释放
#if GOOGLE_PROTOBUF_VERSION >= 3000000
    _arena.reset();
