...
public:
    ...
    static void reconstruct(T& value, A allocator, Args&&... args) noexcept {
        ReusableTraits<T>::reconstruct(value, allocator, ::std::forward<Args>(args)...);
    }

    // 提取实例容量，更新到AllocationMetadata
    template <typename T, typename... Args>
    static void update_allocation_metadata(const T& value, AllocationMetadata<T>& meta) noexcept {
        ReusableTraits<T>::update_allocation_metadata(value, meta);
    }

    // 根据AllocationMetadata重建一个实例
    template <typename T, typename A, typename... Args>
    static void construct_with_allocation_metadata(T* ptr, A allocator, const AllocationMetadata<T>& meta) noexcept {
    ...
    template <typename T, typename A, typename... Args>
    static T* create_with_allocation_metadata(A allocator, const AllocationMetadata<T>& meta) noexcept {
...
private:
    ...
    template <typename T, typename A, typename ::std::enable_if<
        ClearInvocable<T>::value, int>::type = 0>
    static void call_reconstruct(T& value, A) noexcept {
        value.clear();
    }
    template <typename T, typename A, typename ::std::enable_if<
        !ClearInvocable<T>::value
        && ClearUpperInvocable<T>::value, int>::type = 0>
    static void call_reconstruct(T& value, A) noexcept {
        value.Clear();
    }
    template <typename T, typename A, typename U, typename ::std::enable_if<
        OpEqInvocable<T, U>::value, int>::type = 0>
    ...
    template <typename T, typename A, typename... Args, typename ::std::enable_if<
        !OpEqInvocable<T, Args...>::value
        && AssignInvocable<T, Args...>::value, int>::type = 0>
...
public:
    ...
    using AllocationMetadata = typename ::std::conditional<
        ...
        typename ::std::conditional<
            !::std::is_same<void, Reuse::AllocationMetadataDefined::type<T>>::value,
    ...
    template <typename U = T, typename ::std::enable_if<
        (sizeof(typename ReusableTraits<U>::AllocationMetadata) > 0), int>::type = 0>
    static void update_allocation_metadata(const U& instance,
            typename ReusableTraits<U>::AllocationMetadata& meta) noexcept {
        instance.update_allocation_metadata(meta);
    }
    template <typename U = T, typename ::std::enable_if<
        (sizeof(typename ReusableTraits<U>::AllocationMetadata) == 0), int>::type = 0>
    static void update_allocation_metadata(const U&,
            typename ReusableTraits<U>::AllocationMetadata&) noexcept {
    }

    // 根据AllocationMetadata重建一个实例
    template <typename U = T, typename A, typename ::std::enable_if<
        (sizeof(typename ReusableTraits<U>::AllocationMetadata) > 0), int>::type = 0>
    static void construct_with_allocation_metadata(U* ptr, A allocator,
    ...
    static TT* create_with_allocation_metadata(
        ...
        ReusableTraits<TT>::construct_with_allocation_metadata(
                instance, allocator, meta);
...
class ReusableTraits<::std::basic_string<char, ::std::char_traits<char>, A>> :
    public BasicReusableTraits<ReusableTraits<::std::basic_string<char, ::std::char_traits<char>, A>>> {
private:
...
#if __cplusplus < 201703L
template <typename A>
constexpr bool ReusableTraits<::std::basic_string<char, ::std::char_traits<char>, A>>::REUSABLE;
#endif // __cplusplus < 201703L
