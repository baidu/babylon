...
template <typename C, typename CH = ::std::char_traits<C>,
          typename R = MonotonicBufferResource>
class MonotonicBasicString
    : public ::std::basic_string<C, CH, MonotonicAllocator<C, R>> {
 private:
 ...
 public:
  ...
  inline MonotonicBasicString(MonotonicBasicString&& other,
  ...
  template <typename A>
  inline MonotonicBasicString(const ::std::basic_string<C, CH, A>& other,
                              typename Base::allocator_type allocator) noexcept
      : Base(other.begin(), other.end(), allocator) {}

  template <typename A>
  inline MonotonicBasicString& operator=(
      const ::std::basic_string<C, ::std::char_traits<C>, A>& other) noexcept {
...
using SwissString =
    MonotonicBasicString<char, ::std::char_traits<char>, SwissMemoryResource>;
...
inline bool operator==(
    const MonotonicBasicString<CH, ::std::char_traits<CH>, R>& left,
    const MonotonicBasicString<CH, ::std::char_traits<CH>, RR>&
        right) noexcept {
  return BasicStringView<CH>(left) == BasicStringView<CH>(right);
}
...
class ReusableTraits<MonotonicBasicString<CH, ::std::char_traits<CH>, R>>
    : public BasicReusableTraits<
          ReusableTraits<MonotonicBasicString<CH, ::std::char_traits<CH>, R>>> {
 private:
...
constexpr bool ReusableTraits<
    MonotonicBasicString<CH, ::std::char_traits<CH>, R>>::REUSABLE;
