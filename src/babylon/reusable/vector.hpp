...
template <typename T, typename U, typename A>
inline ReusableVector<T, MonotonicAllocator<U, A>>::ReusableVector(
        allocator_type allocator) noexcept : _allocator {allocator} {}
...
inline ReusableVector<T, MonotonicAllocator<U, A>>::ReusableVector(
        size_type count, allocator_type allocator) noexcept :
            _allocator(allocator), _data(_allocator.allocate(count)),
            _size(count), _constructed_size(count), _capacity(count) {
...
inline ReusableVector<T, MonotonicAllocator<U, A>>::ReusableVector(
        IT first, IT last, allocator_type allocator) noexcept :
            _allocator {allocator}, _size(::std::distance(first, last)),
            _constructed_size {_size}, _capacity {_size} {
...
ReusableVector<T, MonotonicAllocator<U, A>>::operator[](size_type pos) noexcept {
...
ReusableVector<T, MonotonicAllocator<U, A>>::operator[](size_type pos) const noexcept {
    assert(pos < _size && "index overflow");
    return _data[pos];
}
...
template <typename T, typename U, typename A>
inline typename ReusableVector<T, MonotonicAllocator<U, A>>::const_reverse_iterator
...
template <typename T, typename U, typename A>
inline bool
ReusableVector<T, MonotonicAllocator<U, A>>::empty() const noexcept {
    return _size == 0;
}
...
ReusableVector<T, MonotonicAllocator<U, A>>::insert(
        const_iterator pos, V&& value) noexcept {
...
ReusableVector<T, MonotonicAllocator<U, A>>::insert(const_iterator pos,
    ...
    for (size_type i = index; i < reconstruct_end_size; ++i) {
        ValueReusableTraits::reconstruct(
            _data[i], _allocator, value);
    }
...
inline typename ReusableVector<T, MonotonicAllocator<U, A>>::iterator
ReusableVector<T, MonotonicAllocator<U, A>>::insert(
        const_iterator pos, IT first, IT last) noexcept {
    size_type index = &*pos - _data;
    auto count = ::std::distance(first, last);
    auto reconstruct_end_size = prepare_for_insert(index, count);
    for (size_type i = index; i < reconstruct_end_size; ++i) {
        ValueReusableTraits::reconstruct(
            _data[i], _allocator, *first++);
    }
    for (size_type i = reconstruct_end_size; i < index + count; ++i) {
        _allocator.construct(&_data[i], *first++);
        ++_constructed_size;
...
inline void ReusableVector<T, MonotonicAllocator<U, A>>::emplace_back(
    ...
    if (_constructed_size > _size) {
        ValueReusableTraits::reconstruct(_data[_size++], _allocator, ::std::forward<Args>(args)...);
    } else {
...
inline ReusableVector<T, MonotonicAllocator<U, A>>::ReusableVector(
        ...
        allocator_type allocator) noexcept :
            ...
            _size(0), _constructed_size(metadata.capacity),
...
template <typename T, typename U, typename A>
inline void ReusableVector<T, MonotonicAllocator<U, A>>::update_allocation_metadata(
        AllocationMetadata& metadata) const noexcept {
    metadata.capacity = ::std::max(_constructed_size, metadata.capacity);
    for (size_type i = 0; i < _constructed_size; ++i) {
        ValueReusableTraits::update_allocation_metadata(
                _data[i], metadata.value_metadata);
    }
}
