...
class ReusableVector<T, MonotonicAllocator<U, A>> {
 ...
 private:
  ...
  template <typename... Args, typename = typename ::std::enable_if<
                                  (sizeof...(Args) > 0) ||
                                  SerializeTraits<T>::SERIALIZABLE>::type>
  ...
  template <typename... Args,
            typename = typename ::std::enable_if<
                (sizeof...(Args) > 0) ||
                SerializeTraits<T>::SERIALIZED_SIZE_CACHED>::type>
  inline size_t serialized_size_cached() const noexcept {
    size_t size = 0;
    for (const auto& one_value : *this) {
      size +=
          SerializationHelper::serialized_size_cached_packed_field(one_value);
    }
    return size;
  }
...
template <typename T, typename A>
constexpr bool
    is_trivially_destructible<::babylon::ReusableVector<T, A>>::value;
