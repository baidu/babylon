...
// 为了方便这种探测，也提供了std::uses_allocator<T, A>用于判定T类型是否遵守了该协议
// 但是由于历史原因，这个协议设计很复杂
...
public:
    ...
    template <typename P, typename A, typename... FArgs, typename... SArgs, typename = typename ::std::enable_if<
        PairConstructible<P, A, ::std::piecewise_construct_t, ::std::tuple<FArgs...>, ::std::tuple<SArgs...>>::value>::type>
...
// 典型用法为支持单调分配器的特化
// template <typename T>
// class MonotonicAllocator<T, CustumMemoryResource> : public BasicMonotonicAllocator<T, MonotonicAllocator<T, CustumMemoryResource>> {
//   ...
// };
//
// 特殊情况下，也支持非MonotonicAllocator类型使用
// template <typename T>
// class CustomAllocator<T> : public BasicMonotonicAllocator<T, CustomAllocator<T>> {
//   ...
// };
...
private:
    ...
    template <template <typename, typename...> class AllocTemplate,
              typename V, typename U, typename... Args>
...
public:
    ...
    inline void deallocate_bytes(
            void* ptr, size_t nbytes, size_t alignment) noexcept;
...
protected:
    ...
    template <typename U, typename... Args>
    using BaseConstructible = ConstructInvocable<BasicMonotonicAllocator<T, A>, U*, Args...>;
...
public:
    ...
    inline MonotonicAllocator& operator=(const MonotonicAllocator&) noexcept = default;
    inline ~MonotonicAllocator() noexcept = default;
...
public:
    ...
    template <typename U, typename... Args, typename = typename ::std::enable_if<
        UsesAllocatorConstructor::Constructible<U, ::std::pmr::polymorphic_allocator<U>, Args...>::USES_ALLOCATOR
        >::type>
    inline void construct(U* ptr, Args&&... args) {
        UsesAllocatorConstructor::construct(ptr, ::std::pmr::polymorphic_allocator<U>(this->_resource), ::std::forward<Args>(args)...);
    }
#endif // __cpp_lib_memory_resource >= 201603L
...
struct is_trivially_destructible<
        ::std::basic_string<CH, CT,
                            ::babylon::MonotonicAllocator<U, A>>> {
...
constexpr bool is_trivially_destructible<
    ::std::basic_string<CH, CT,
                        ::babylon::MonotonicAllocator<U, A>>>::value;
