...
MemoryLocker& MemoryLocker::set_check_interval(std::chrono::seconds interval) noexcept {
...
void MemoryLocker::check_and_lock() noexcept {
    ...
    while (::std::getline(ifs, line)) {
        ...
        // 区域记入待锁定集合
        regions.emplace(reinterpret_cast<const void*>(region_start), region_end - region_start);
    }

    lock_regions(::std::move(regions));
}

void MemoryLocker::lock_regions(::std::unordered_map<const void*, size_t>&& regions) noexcept {
    // 无变化，不做处理
    ...
    BABYLON_LOG(INFO) << "mlock " << _locked_regions.size() << " regions with bytes " << _locked_bytes << " errno " << _last_errno;
}
