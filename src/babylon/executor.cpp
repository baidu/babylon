...
int AlwaysUseNewThreadExecutor::invoke(MoveOnlyFunction<void(void)>&& function) noexcept {
    ::std::thread(::std::bind([] (MoveOnlyFunction<void(void)>& function) {
        function();
    }, ::std::move(function))).detach();
    return 0;
}
...
int ThreadPoolExecutor::initialize(size_t worker_num, size_t queue_capacity) noexcept {
    if (!_threads.empty()) {
        return -1;
    }
...
int ThreadPoolExecutor::invoke(MoveOnlyFunction<void(void)>&& function) noexcept {
