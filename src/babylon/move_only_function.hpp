...
inline MoveOnlyFunction<R(Args...)>::MoveOnlyFunction(C&& callable) noexcept :
    _function(internal::move_only_function::MoveOnlyCallable<C, R, Args...>(::std::move(callable))) {}

template <typename R, typename... Args>
template <typename C, typename>
inline MoveOnlyFunction<R(Args...)>& MoveOnlyFunction<R(Args...)>::operator=(C&& callable) noexcept {
    _function = internal::move_only_function::MoveOnlyCallable<C, R, Args...>(::std::move(callable));
    return *this;
}
...
template <typename T>
inline UncomposableBindArgument<typename ::std::decay<T>::type> uncomposable_bind_argument(T&& value) {
    return UncomposableBindArgument<typename ::std::decay<T>::type>(::std::forward<T>(value));
}
