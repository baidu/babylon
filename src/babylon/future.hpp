...
struct NonVoid {
    ...
    typedef typename ::std::conditional<::std::is_void<T>::value, S, T>::type type;
};

// 当T& value就绪后，统一执行回调的函数，便于不同的函数签名
// 支持C = R(T&&)，R(const T&)，R(T&)，R()的情况
template <typename C, typename T, typename ::std::enable_if<
    IsInvocable<C, T&&>::value, int32_t>::type = 0>
inline auto run_callback(C& callback, T& value) noexcept -> decltype(callback(::std::move(value))) {
    return callback(::std::move(value));
}
template <typename C, typename T, typename ::std::enable_if<
    !IsInvocable<C, T&&>::value && IsInvocable<C, T&>::value, int32_t>::type = 0>
inline auto run_callback(C& callback, T& value) noexcept -> decltype(callback(value)) {
    return callback(value);
}
template <typename C, typename T, typename ::std::enable_if<IsInvocable<C>::value, int32_t>::type = 0>
...
struct IsCompatibleCallback {
    static constexpr bool value = IsInvocable<C, typename NonVoid<T>::type&&>::value
...
template <typename T, typename M, typename C, typename U,
          typename ::std::enable_if<!::std::is_void<
              typename InvokeResult<
                  decltype(&run_callback<C, U>), C&, U&>::type
              >::value, int>::type = 0>
inline void run_callback(Promise<T, M>& promise, C& callback, U& value) noexcept {
...
public:
    ...
    typedef typename ::std::aligned_storage<sizeof(ValueType), alignof(ValueType)>::type StorageType;
    // 回调链节点
    struct CallbackNode {
        template <typename C>
    ...
    template <typename... Args, typename = typename ::std::enable_if<::std::is_constructible<ValueType, Args...>::value>::type>
    ...
    // 注册回调，就绪后会激活回调函数
    template <typename C, typename = typename ::std::enable_if<IsCompatibleCallback<C>::value>::type>
...
template <typename T, typename M>
inline bool FutureContext<T, M>::ready(::std::memory_order memory_order) const noexcept {
...
inline void FutureContext<T, M>::set_value(Args&&... args) {
    ...
    auto head = seal();
    // 唤醒阻塞等待者
    auto waiter_num = _futex.value().exchange(READY_MASK, ::std::memory_order_release);
    if (waiter_num > 0) {
        _futex.wake_all();
    }
    // 运行回调链
...
template <typename T, typename M>
inline typename FutureContext<T, M>::ValueType& FutureContext<T, M>::get() noexcept {
...
inline void FutureContext<T, M>::on_finish(C&& callback) noexcept {
    ...
    auto* node = new CallbackNode([this, callback = ::std::forward<C>(callback)] () mutable {
        internal::future::run_callback(callback, value());
    });
#else // __cplusplus < 201402L
    auto* node = new CallbackNode(::std::bind(internal::future::run_callback<C, ValueType>,
        uncomposable_bind_argument(::std::forward<C>(callback)), ::std::ref(value())));
#endif // __cplusplus
...
template <typename T, typename M>
inline bool FutureContext<T, M>::has_callback(::std::memory_order memory_order) const noexcept {
...
template <typename T, typename M>
inline constexpr bool FutureContext<T, M>::is_sealed(CallbackNode* head) noexcept {
...
void FutureContext<T, M>::wait_slow() noexcept {
...
template <typename T, typename M>
ABSL_ATTRIBUTE_NOINLINE 
bool FutureContext<T, M>::wait_for_slow(
...
template <typename T, typename M>
template <typename C, typename>
inline typename Future<T, M>::template ThenFuture<C, M>
Future<T, M>::then(C&& callback) noexcept {
    ThenPromise<C, M> promise;
    auto future = promise.get_future();
#if __cplusplus >= 201402L
    on_finish([promise = ::std::move(promise), callback = ::std::forward<C>(callback)] (ResultType& value) mutable {
        internal::future::run_callback(promise, callback, value);
    });
#else // __cplusplus < 201402L
    on_finish(::std::bind(internal::future::run_callback<ThenType<C>, M, C, ResultType>,
        ::std::move(promise), uncomposable_bind_argument(::std::forward<C>(callback)), ::std::placeholders::_1));
#endif // __cplusplus
    return future;
}

template <typename T, typename M>
inline Future<T, M>::Future(::std::shared_ptr<FutureContext<T, M>>& context) noexcept :
    _context(context) {}
// Future end
///////////////////////////////////////////////////////////////////////////////
...
inline void Promise<T, M>::set_value(Args&&... args) {
    if (ABSL_PREDICT_TRUE(_context && !_context->ready(::std::memory_order_relaxed))) {
