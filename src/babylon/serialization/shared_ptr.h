...
template <typename T>
class SerializeTraits<::std::shared_ptr<T>> : public BasicSerializeTraits<SerializeTraits<::std::shared_ptr<T>>> {
private:
...
public:
    static constexpr bool SERIALIZABLE = SerializeTraits<MutableType>::SERIALIZABLE;
    ...
    static constexpr bool PRINT_AS_OBJECT = SerializeTraits<MutableType>::PRINT_AS_OBJECT;
    ...
    static size_t calculate_serialized_size(const Value& value) noexcept {
        return value ?
            SerializeTraits<MutableType>::calculate_serialized_size(*value) : 0;
    }

    static size_t serialized_size_cached(const Value& value) noexcept {
        return value ? SerializeTraits<MutableType>::serialized_size_cached(*value) : 0;
    }
...
template <typename T>
constexpr bool SerializeTraits<::std::shared_ptr<T>>::SERIALIZABLE;
