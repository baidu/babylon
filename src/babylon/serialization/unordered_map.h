...
class SerializeTraits<::std::unordered_map<K, V, H, E, A>>
 ...
 public:
  static constexpr bool SERIALIZABLE =
      SerializeTraits<K>::SERIALIZABLE && SerializeTraits<V>::SERIALIZABLE;
  ...
  static bool deserialize(CodedInputStream& is, Value& value) noexcept {
    ...
    while (is.GetDirectBufferPointer(&data, &size)) {
      ...
      if (ABSL_PREDICT_FALSE(
              !SerializationHelper::deserialize_packed_field(is, k))) {
        return false;
      }
      V v;
      if (ABSL_PREDICT_FALSE(
              !SerializationHelper::deserialize_packed_field(is, v))) {
        return false;
      }
  ...
  static size_t serialized_size_cached(const Value& value) noexcept {
    size_t size = 0;
    for (const auto& one_value : value) {
      size += SerializationHelper::serialized_size_cached_packed_field(
          one_value.first);
      size += SerializationHelper::serialized_size_cached_packed_field(
          one_value.second);
    }
  ...
  static bool print(const Value& value, PrintStream& ps) noexcept {
    ...
    if (ABSL_PREDICT_FALSE(!ps.print_raw("[") || !ps.indent())) {
      return false;
    }
    for (const auto& one_value : value) {
      if (ABSL_PREDICT_FALSE(!ps.start_new_line() || !ps.print_raw("{") ||
                             !ps.indent() || !ps.start_new_line())) {
        return false;
      }
      if (ABSL_PREDICT_FALSE(
              !SerializationHelper::print_field("key", one_value.first, ps))) {
        return false;
      }
      if (ABSL_PREDICT_FALSE(!SerializationHelper::print_field(
              "value", one_value.second, ps))) {
        return false;
      }
      if (ABSL_PREDICT_FALSE(!ps.outdent() || !ps.print_raw("}"))) {
        return false;
      }
    }
    if (ABSL_PREDICT_FALSE(!ps.outdent() || !ps.start_new_line() ||
                           !ps.print_raw("]"))) {
      return false;
    }
    return true;
...
template <typename K, typename V, typename H, typename E, typename A>
constexpr bool
    SerializeTraits<::std::unordered_map<K, V, H, E, A>>::SERIALIZABLE;
