...
struct SerializationHelper {
    ...
    BABYLON_HAS_STATIC_MEMBER_CHECKER(BABYLON_SERIALIZED_SIZE_COMPLEXITY, HasSerializedSizeComplexity)
    ...
    BABYLON_DECLARE_MEMBER_INVOCABLE(calculate_serialized_size, CalculateSerializedSizeInvocable)
    BABYLON_DECLARE_MEMBER_INVOCABLE(serialized_size_cached, SerializedSizeCachedInvocable)
    ...
    template <typename T, typename ::std::enable_if<
        serialize_invocable<T>(), int>::type = 0>
    ...
    template <typename T, typename ::std::enable_if<
        deserialize_invocable<T>(), int>::type = 0>
    ...
    calculate_serialized_size_invocable() noexcept {
        return CalculateSerializedSizeInvocable<T>::value;
    }
    template <typename T, typename ::std::enable_if<
        calculate_serialized_size_invocable<T>(), int>::type = 0>
    inline static size_t calculate_serialized_size(
    ...
    template <typename T, typename ::std::enable_if<
        SerializedSizeCachedInvocable<T>::value, int>::type = 0>
    ...
    inline static bool print(const T&, PrintStream& ps) noexcept {
        return ps.print_raw("<type '")
            && ps.print_raw(TypeId<T>::ID.name)
            && ps.print_raw("'>");
    }

    template <typename T, typename ::std::enable_if<
        HasSerializedSizeComplexity<T>::value, int>::type = 0>
    ...
    template <typename T, typename ::std::enable_if<
        HasPrintAsObject<T>::value, int>::type = 0>
...
#undef BABYLON_TMP_CLZ
    ...
    template <typename T>
    inline static constexpr uint32_t make_tag(
    ...
    inline static void serialize_field(
            uint32_t tag, const T& value,
            CodedOutputStream& os, const C& field_cache) noexcept {
        // 尝试优先使用外部缓存
    ...
    template <typename T>
    inline static bool deserialize_field(
            uint32_t tag, CodedInputStream& is, T& value) noexcept {
#ifndef NDEBUG
    ...
    inline size_t static calculate_serialized_size_field(
            uint32_t tag_size, const T& value,
            C& field_cache) noexcept {
    ...
    // 主要用于支持容器类的实现
    template <typename T>
    inline static void serialize_packed_field(
            const T& value, CodedOutputStream& os) noexcept {
        if CONSTEXPR_SINCE_CXX17 (SerializeTraits<T>::WIRE_TYPE ==
                WireFormatLite::WIRETYPE_LENGTH_DELIMITED) {
    ...
    template <typename T>
    inline static bool deserialize_packed_field(
    ...
    inline static bool print_field(StringView field_name, const T& value,
        ...
        if (!SerializeTraits<T>::PRINT_AS_OBJECT) {
            return ps.print_raw(field_name)
                && ps.print_raw(": ")
                && SerializeTraits<T>::print(value, ps)
                && ps.start_new_line();
        } else {
            return ps.print_raw(field_name)
                && ps.print_raw(" {")
    ...
    static bool consume_unknown_field(uint32_t tag, CodedInputStream& is) noexcept;
};
...
public:
    ...
    static constexpr WireType WIRE_TYPE = WireFormatLite::WIRETYPE_LENGTH_DELIMITED;
    ...
    static constexpr bool PRINT_AS_OBJECT = SerializationHelper::print_as_object<T>();
...
template <typename T>
bool Serialization::serialize_to_coded_stream(
        const T& value, CodedOutputStream& os) noexcept {
...
template <typename T>
bool Serialization::serialize_to_string(
        const T& value, ::std::string& s) noexcept {
    s.clear();
...
template <typename T>
bool Serialization::serialize_to_array_with_cached_size(
        const T& value, void* buffer, size_t size) noexcept {
    ::google::protobuf::io::ArrayOutputStream as {buffer,
...
template <typename T>
bool Serialization::parse_from_coded_stream(
        CodedInputStream& is, T& value) noexcept {
    return SerializeTraits<T>::deserialize(is, value);
}

template <typename T>
bool Serialization::parse_from_string(
        const ::std::string& s, T& value) noexcept {
    return parse_from_array(s.c_str(), s.size(), value);
}

template <typename T>
bool Serialization::parse_from_array(
...
bool Serialization::DefaultSerializer<T>::parse_from_coded_stream(
    ...
    return Serialization::parse_from_coded_stream(
            is, mutable_value(instance));
}

template <typename T>
bool Serialization::DefaultSerializer<T>::parse_from_string(
        const ::std::string& str, Any& instance) noexcept {
    return Serialization::parse_from_string(
            str, mutable_value(instance));
}

template <typename T>
bool Serialization::DefaultSerializer<T>::parse_from_array(
        const char* data, size_t size, Any& instance) noexcept {
    return Serialization::parse_from_array(
            data, size, mutable_value(instance));
}
