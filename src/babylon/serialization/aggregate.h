...
#define BABYLON_SERIALIZABLE(...)                  \
  __BABYLON_SERIALIZABLE_IMPL(                     \
      , BOOST_PP_VARIADIC_SEQ_TO_SEQ(BOOST_PP_IF(  \
            BOOST_PP_IS_BEGIN_PARENS(__VA_ARGS__), \
            __BABYLON_SERIALIZABLE_REMOVE_FIRST,   \
            __BABYLON_SERIALIZABLE_AUTO_TAG_FROM)(1, ##__VA_ARGS__)))

#define BABYLON_SERIALIZABLE_WITH_BASE(base, ...)                              \
  __BABYLON_SERIALIZABLE_IMPL(                                                 \
      BOOST_PP_VARIADIC_SEQ_TO_SEQ(BOOST_PP_IF(                                \
          BOOST_PP_IS_BEGIN_PARENS(base), __BABYLON_SERIALIZABLE_REMOVE_FIRST, \
          __BABYLON_SERIALIZABLE_AUTO_TAG_FROM)(1, base)),                     \
      BOOST_PP_VARIADIC_SEQ_TO_SEQ(BOOST_PP_IF(                                \
          BOOST_PP_IS_BEGIN_PARENS(base), __BABYLON_SERIALIZABLE_REMOVE_FIRST, \
          __BABYLON_SERIALIZABLE_AUTO_TAG_FROM)(2, ##__VA_ARGS__)))

#define __BABYLON_SERIALIZABLE_REMOVE_FIRST(first, ...) __VA_ARGS__

#define __BABYLON_SERIALIZABLE_AUTO_TAG_FROM(begin_field_number, ...) \
  BOOST_PP_SEQ_FOR_EACH_I_R(1, __BABYLON_SERIALIZABLE_AUTO_TAG_EACH,  \
                            begin_field_number,                       \
                            BOOST_PP_VARIADIC_TO_SEQ(__VA_ARGS__))

#define __BABYLON_SERIALIZABLE_AUTO_TAG_EACH(r, begin_field_number, index, \
                                             member)                       \
...
#define __BABYLON_SERIALIZABLE_IMPL(base, fields)                                                            \
  ...
  static constexpr int BABYLON_SERIALIZED_SIZE_COMPLEXITY =                                                  \
      (true BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_TRIVIAL_BASE, _,                               \
                                    base)                                                                    \
           BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_TRIVIAL_FIELD, _,                               \
                                   fields))                                                                  \
  ...
  size_t calculate_serialized_size() const noexcept {                                                        \
    size_t __babylon_tmp_size = 0 BOOST_PP_SEQ_FOR_EACH_R(                                                   \
        1, __BABYLON_SERIALIZABLE_CAL_SIZE_BASE, _, base)                                                    \
        BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CAL_SIZE_FIELD, _,                                 \
                                fields);                                                                     \
  ...
  static constexpr bool __babylon_need_cache() noexcept {                                                    \
    return sizeof(__babylon_cached_serialized_size) >                                                        \
           0 BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CACHED_BASE, _,                               \
                                     base)                                                                   \
               BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CACHED_FIELD,                               \
                                       _, fields);                                                           \
  }                                                                                                          \
  ...
  size_t serialized_size_cached(Args...) const noexcept {                                                    \
    ...
    /* 否则累加各成员 */                                                                              \
    return 0 BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CH_SIZE_BASE,                                 \
                                     _, base)                                                                \
        BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CH_SIZE_FIELD, _,                                  \
                                fields);                                                                     \
  }                                                                                                          \
  /* 实现SerializeTraits约定的序列化接口 */                                                        \
  void serialize(::google::protobuf::io::CodedOutputStream& os)                                              \
      const noexcept {                                                                                       \
  ...
  bool deserialize(::google::protobuf::io::CodedInputStream& is) noexcept {                                  \
    ...
    while (                                                                                                  \
        is.GetDirectBufferPointer(&__babylon_tmp_data, &__babylon_tmp_size)) {                               \
      /* 对齐Protocol Buffer协议 */                                                                      \
      /* message = [field] */                                                                                \
      /* field = tag data */                                                                                 \
      ...
      switch (__babylon_tmp_tag >> 3) {                                                                      \
        BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_DER_BASE, _, base)                                 \
        BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_DER_FIELD, _,                                      \
                                fields)                                                                      \
        /* 对于未知的field_number尝试跳过 */                                                        \
        default:                                                                                             \
          if (ABSL_PREDICT_FALSE(                                                                            \
                  !::babylon::SerializationHelper::consume_unknown_field(                                    \
  ...
  bool print(::babylon::Serialization::PrintStream& ps) const noexcept {                                     \
    (void)ps;                                                                                                \
    return true BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_PRINT_BASE,                                \
                                        _, base)                                                             \
        BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_PRINT_FIELD, _,                                    \
                                fields);                                                                     \
  }                                                                                                          \
  /* 对于COMPLEX成员，定义配套的成员缓存 */                                                    \
  BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CACHED_SIZE_BASE, _, base)                               \
  BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_CACHED_SIZE_FIELD, _,                                    \
                          fields)                                                                            \
  /* 统计SIMPLE成员数量，过多时定义整体缓存 */                                               \
  mutable ::std::conditional<                                                                                \
      (10 > (0 BOOST_PP_SEQ_FOR_EACH_R(1, __BABYLON_SERIALIZABLE_COUNT_FIELD,                                \
                                       _, fields))),                                                         \
      ::babylon::ZeroSized, uint32_t>::type __babylon_cached_serialized_size {                               \
      0};                                                                                                    \
...
#define __BABYLON_SERIALIZABLE_SER_BASE(r, data, base)                        \
  ::babylon::SerializationHelper::serialize_field(                            \
      ::babylon::SerializationHelper::make_tag<BOOST_PP_TUPLE_ELEM(0, base)>( \
...
#define __BABYLON_SERIALIZABLE_DER_BASE(r, data, base)                         \
  case BOOST_PP_TUPLE_ELEM(1, base):                                           \
    if (ABSL_PREDICT_FALSE(!::babylon::SerializationHelper::deserialize_field( \
            __babylon_tmp_tag, is,                                             \
            *static_cast<BOOST_PP_TUPLE_ELEM(0, base)*>(this)))) {             \
      return false;                                                            \
    }                                                                          \
    break;
...
#define __BABYLON_SERIALIZABLE_DER_FIELD(r, data, field)                       \
  case BOOST_PP_TUPLE_ELEM(1, field):                                          \
    if (ABSL_PREDICT_FALSE(!::babylon::SerializationHelper::deserialize_field( \
            __babylon_tmp_tag, is, BOOST_PP_TUPLE_ELEM(0, field)))) {          \
      return false;                                                            \
    }                                                                          \
    break;
...
#define __BABYLON_SERIALIZABLE_CAL_SIZE_BASE(r, data, base)              \
  +::babylon::SerializationHelper::calculate_serialized_size_field(      \
      ::babylon::SerializationHelper::make_tag_size<BOOST_PP_TUPLE_ELEM( \
          0, base)>(BOOST_PP_TUPLE_ELEM(1, base)),                       \
...
#define __BABYLON_SERIALIZABLE_CACHED_SIZE_BASE(r, data, base)                \
  mutable ::std::conditional<                                                 \
      ...
      uint32_t, ::babylon::ZeroSized>::type __babylon_cached_size_base {0};
...
#define BABYLON_COMPATIBLE(...) BABYLON_SERIALIZABLE(__VA_ARGS__)
