...
class Serialization {
 ...
 public:
  ...
  template <typename T>
  static bool serialize_to_coded_stream(const T& value,
  ...
  template <typename T>
  static bool serialize_to_array_with_cached_size(const T& value, void* buffer,
  ...
  // 否则恒定失败
  template <typename T>
  static bool parse_from_coded_stream(CodedInputStream& is, T& value) noexcept;
  ...
  // 否则恒定失败
  template <typename T>
  static bool parse_from_string(const ::std::string& s, T& value) noexcept;
  ...
  // 否则恒定失败
  template <typename T>
  static bool parse_from_array(const char* data, size_t size,
                               T& value) noexcept;
 ...
 public:
  ...
  class Serializer {
   public:
    ...
    virtual bool parse_from_coded_stream(CodedInputStream& is,
                                         Any& instance) noexcept = 0;
    virtual bool parse_from_string(const ::std::string&,
                                   Any& instance) noexcept = 0;
    virtual bool parse_from_array(const char* data, size_t size,
                                  Any& value) noexcept = 0;
    virtual bool print_to_stream(const Any& value,
                                 ZeroCopyOutputStream& os) noexcept = 0;
    virtual bool print_to_string(const Any& value,
                                 ::std::string& s) noexcept = 0;
  };
  ...
  class DefaultSerializer : public Serializer {
    ...
    virtual bool parse_from_coded_stream(CodedInputStream& is,
                                         Any& instance) noexcept override;
    virtual bool parse_from_string(const ::std::string& str,
                                   Any& instance) noexcept override;
    virtual bool parse_from_array(const char* data, size_t size,
                                  Any& instance) noexcept override;
    virtual bool print_to_stream(const Any& instance,
                                 ZeroCopyOutputStream& os) noexcept override;
    virtual bool print_to_string(const Any& instance,
                                 ::std::string& s) noexcept override;
  };
...
class SerializeTraits : public BasicSerializeTraits<SerializeTraits<T>> {
 ...
 public:
  ...
  static constexpr int SERIALIZED_SIZE_COMPLEXITY =
      Base::SERIALIZED_SIZE_COMPLEXITY;
...
template <typename T, typename E>
constexpr
    typename SerializeTraits<T, E>::WireType SerializeTraits<T, E>::WIRE_TYPE;
template <typename T, typename E>
...
#define BABYLON_REGISTER_SERIALIZER(cls) \
  BABYLON_REGISTER_SERIALIZER_IMPL(      \
      cls, BOOST_PP_CAT(BabylonAutoRegister, __COUNTER__))
