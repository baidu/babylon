...
void ClosureContext::finish(int32_t error_code) noexcept {
    ...
    if (mark_finished(error_code, callback)) {
        if (callback != nullptr && ABSL_PREDICT_FALSE(0 != invoke(callback))) {
            BABYLON_LOG(WARNING) << "closure[" << this << "] invoke callback[" << callback
...
bool ClosureContext::mark_finished(int32_t error_code, Closure::Callback*& callback) noexcept {
    callback = _callback.load(::std::memory_order_relaxed);
    do {
        if (ABSL_PREDICT_FALSE(callback == SEALED_CALLBACK)) {
            return false;
        }
    } while (ABSL_PREDICT_FALSE(!_callback.compare_exchange_weak(callback,
...
template <typename S>
ClosureContextImplement<S>::ClosureContextImplement(GraphExecutor& executor) noexcept :
