...
GraphVertexBuilder::option<::babylon::Any>() const noexcept {
    return &_option;
}
...
inline ::std::ostream& operator<<(::std::ostream& os, const GraphEmitBuilder& emit) noexcept {
    os << "dependency[";
    if (emit.name().empty()) {
        os << emit.index();
    } else {
...
inline ::std::ostream& operator<<(::std::ostream& os, const GraphDependencyBuilder& dependency) noexcept {
    os << "dependency[";
