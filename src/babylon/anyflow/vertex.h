...
public:
    ...
    inline GraphVertexClosure(ClosureContext& closure, GraphVertex& vertex) noexcept;
...
#define __ANYFLOW_TYPE_FOR_NAME(name) \
...
#define __ANYFLOW_MEMBER(r, data, args) \
    BOOST_PP_IF(BOOST_PP_EQUAL(BOOST_PP_TUPLE_ELEM(0, args), 0), \
        __ANYFLOW_DEPEND args, \
        __ANYFLOW_EMIT args \
    )

#define __ANYFLOW_DEPEND(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
        ssize_t __anyflow_index_for_##name {-1}; \
        BOOST_PP_IF(is_channel, \
            BOOST_PP_IF(is_mutable, \
                ::babylon::anyflow::MutableChannelConsumer<__ANYFLOW_TYPE_FOR_NAME(name)> name;, \
                ::babylon::anyflow::ChannelConsumer<__ANYFLOW_TYPE_FOR_NAME(name)> name; \
            ), \
            BOOST_PP_IF(is_mutable, \
                __ANYFLOW_TYPE_FOR_NAME(name)* name {nullptr};, \
...
#define __ANYFLOW_EMIT(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
        ssize_t __anyflow_index_for_##name {-1}; \
        BOOST_PP_IF(is_channel, \
            ::babylon::anyflow::OutputChannel<__ANYFLOW_TYPE_FOR_NAME(name)> name;, \
            ::babylon::anyflow::OutputData<__ANYFLOW_TYPE_FOR_NAME(name)> name; \
        ), \
        std::vector<::babylon::anyflow::OutputData<__ANYFLOW_TYPE_FOR_NAME(name)>> name; \
    )

#define __ANYFLOW_DECLARE(r, data, args) \
    BOOST_PP_IF(BOOST_PP_EQUAL(BOOST_PP_TUPLE_ELEM(0, args), 0), \
        __ANYFLOW_DECLARE_DEPEND args, \
        __ANYFLOW_DECLARE_EMIT args)

#define __ANYFLOW_DECLARE_DEPEND(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    { \
        BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
            auto index = vertex().index_for_named_dependency(#name); \
            if (index < 0 && essential_level > 0) { \
                BABYLON_LOG(WARNING) << "no depend bind to " #name " for " << vertex(); \
                return -1; \
            } \
            if (index >= 0) { \
...
#define __ANYFLOW_DECLARE_EMIT(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    { \
        BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
            auto index = vertex().index_for_named_emit(#name); \
            if (index < 0) { \
                BABYLON_LOG(WARNING) << "no emit bind to " #name " for " << vertex(); \
                return -1; \
            } \
...
#define __ANYFLOW_PREPARE(r, data, args) \
    BOOST_PP_IF(BOOST_PP_EQUAL(BOOST_PP_TUPLE_ELEM(0, args), 0), \
        __ANYFLOW_PREPARE_DEPEND args, \
        __ANYFLOW_PREPARE_EMIT args \
    )

#define __ANYFLOW_PREPARE_DEPEND(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    { \
        BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
            if (__anyflow_index_for_##name >= 0) { \
                ...
                BOOST_PP_IF(is_channel, \
                    BOOST_PP_IF(is_mutable, \
                        auto channel = depend->mutable_channel<__ANYFLOW_TYPE_FOR_NAME(name)>();, \
                        auto channel = depend->channel<__ANYFLOW_TYPE_FOR_NAME(name)>(); \
                    ) \
                    name = channel.subscribe();, \
                    BOOST_PP_IF(is_mutable, \
                        name = depend->mutable_value<__ANYFLOW_TYPE_FOR_NAME(name)>();, \
                ...
                BOOST_PP_IF(is_channel, \
                    BOOST_PP_IF(is_mutable, \
                        ::babylon::anyflow::MutableInputChannel<__ANYFLOW_TYPE_FOR_NAME(name)> channel;, \
                        ::babylon::anyflow::InputChannel<__ANYFLOW_TYPE_FOR_NAME(name)> channel; \
                    ) \
                    name = channel.subscribe();, \
                    name = nullptr; \
                ) \
            ...
            for (size_t index = 0; index < anonymous_depends_size; index++) { \
                ...
                BOOST_PP_IF(BOOST_PP_GREATER(essential_level, 0), \
                    if (!static_cast<bool>(name.at(index))) { \
                        BABYLON_LOG(WARNING) << "anonymous depend data " #name " is empty"; \
                        return -1; \
                    }, \
                ) \
            } \
...
#define __ANYFLOW_PREPARE_EMIT(r, type, name, is_channel, is_mutable, essential_level, is_va_args, ...) \
    { \
        BOOST_PP_IF(BOOST_PP_EQUAL(is_va_args, 0), \
...
#define ANYFLOW_DEPEND_DATA(type, name, ...) ANYFLOW_DEPEND((type, name, 0, 0, ##__VA_ARGS__))
#define ANYFLOW_DEPEND_MUTABLE_DATA(type, name, ...) ANYFLOW_DEPEND((type, name, 0, 1, ##__VA_ARGS__))
#define ANYFLOW_DEPEND_CHANNEL(type, name, ...) ANYFLOW_DEPEND((type, name, 1, 0, ##__VA_ARGS__))
#define ANYFLOW_DEPEND_MUTABLE_CHANNEL(type, name, ...) ANYFLOW_DEPEND((type, name, 1, 1, ##__VA_ARGS__))
#define ANYFLOW_DEPEND_VA_DATA(type, va_args_vec, ...) ANYFLOW_DEPEND((type, va_args_vec, 0, 0, 2, 1))
#define ANYFLOW_DEPEND(args) (__ANYFLOW_FILL_ARGS(BOOST_PP_TUPLE_PUSH_FRONT(args, 0)))
...
#define ANYFLOW_EMIT_VA_DATA(type, va_args_vec, ...) ANYFLOW_EMIT((type, va_args_vec, 0, 0, 2, 1))
#define ANYFLOW_EMIT(args) (__ANYFLOW_FILL_ARGS(BOOST_PP_TUPLE_PUSH_FRONT(args, 1)))

#define __ANYFLOW_FILL_ARGS(args) \
    __ANYFLOW_FILL_IS_VA_ARGS(\
        __ANYFLOW_FILL_ESSENTIAL_LEVEL(\
            __ANYFLOW_FILL_IS_MUTABLE(\
                __ANYFLOW_FILL_IS_CHANNEL(args))))
