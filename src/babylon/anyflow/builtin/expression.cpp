...
public:
    ...
    Operator(size_t result_index) noexcept :
...
protected:
    using GetOperandFunction = const Any& (*)(
...
public:
    UnaryOperator(
            const ValueIndex& result,
            const ValueIndex& operand) noexcept :
...
protected:
    ...
    inline static Any::Type get_caculation_type(
            const Any& loperand, const Any& roperand) noexcept {
...
struct ConditionalExpression : public grammar<const char*, space_type,
    ...
    ConditionalExpression() noexcept : base_type(start, "conditional_expression") {
...
struct NonConditionalExpression : public grammar<const char*, space_type,
    ...
    NonConditionalExpression(expression::Option& option,
    ...
    void on_bool(bool& value, ::boost::spirit::context<
            ::boost::fusion::cons<
                ::std::tuple<size_t, size_t>&,
                ::boost::fusion::nil_>,
            ::boost::fusion::vector0<>>& context) noexcept {
    ...
    void on_double(double& value, ::boost::spirit::context<
            ::boost::fusion::cons<
                ::std::tuple<size_t, size_t>&,
                ::boost::fusion::nil_>,
            ::boost::fusion::vector0<>>& context) noexcept {
    ...
    void on_long(int64_t& value, ::boost::spirit::context<
            ::boost::fusion::cons<
                ::std::tuple<size_t, size_t>&,
                ::boost::fusion::nil_>,
            ::boost::fusion::vector0<>>& context) noexcept {
    ...
    void on_string(::std::string& value, ::boost::spirit::context<
            ::boost::fusion::cons<
                ::std::tuple<size_t, size_t>&,
                ::boost::fusion::nil_>,
            ::boost::fusion::vector0<>>& context) noexcept {
    ...
    void on_variable(::std::string& value, ::boost::spirit::context<
            ::boost::fusion::cons<
                ::std::tuple<size_t, size_t>&,
                ::boost::fusion::nil_>,
            ::boost::fusion::vector0<>>& context) noexcept {
...
int ExpressionProcessor::setup() noexcept {
    ...
    if (vertex().anonymous_dependency_size() != dependency_num) {
        BABYLON_LOG(WARNING) << "dependency num[" << vertex().anonymous_dependency_size()
            << "] != " << expr_option->variable_index_for_dependency.size() << " for " << vertex();
        return -1;
    }
    // 校验输出数目
...
int ExpressionProcessor::process() noexcept {
    ...
    for (size_t i = 0; i < vertex().anonymous_dependency_size(); ++i) {
        ...
        auto dependency_value = vertex().anonymous_dependency(i)->value<::babylon::Any>();
    ...
    for (const auto& dependency : dependencies) {
        if (emits.count(dependency) == 0) {
            if (0 != expend_expression(builder, emits, dependency, dependency)) {
                BABYLON_LOG(WARNING) << "create ExpressionProcessor for [" << dependency << "] failed";
                return -1;
            }
        }
    }
...
int32_t ExpressionProcessor::apply(GraphBuilder& builder,
    ...
    if (emits.count(result_name) > 0) {
        BABYLON_LOG(WARNING) << "result name for expression already exist " << result_name << " = " << expression_string;
        return -1;
    }
    if (0 != expend_expression(builder, emits, result_name, expression_string)) {
        BABYLON_LOG(WARNING) << "expend expression failed for " << result_name << " = " << expression_string;
        return -1;
    }
    return 0;
}
...
int32_t ExpressionProcessor::expend_expression(GraphBuilder& builder,
    ...
    while (!unsolved_expressions.empty()) {
        ...
        if (ret < 0) {
            BABYLON_LOG(WARNING) << "solve expression failed {" << unsolved_name << "} = {" << unsolved_expression << "}";
            return -1;
        } else if (ret == 0) {
            continue;
        }
...
int32_t ExpressionProcessor::expend_non_conditional_expression(GraphBuilder& builder,
    ...
    if (option.operators.empty()) {
        ...
        BABYLON_LOG(WARNING) << "expression without variable nor constant {" << result_name << "} = {"
    ...
    vertex.set_name(std::string("ExpressionProcessor").append(std::to_string(++_s_idx)));
