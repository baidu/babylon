...
template <typename T>
inline ChannelPublisher<T>::ChannelPublisher(
    Topic& topic, GraphData& data) noexcept :
...
template <typename T, typename ::std::enable_if<
    GraphData::Resetable<T>::value, int>::type>
...
template <typename T, typename ::std::enable_if<
    !GraphData::Resetable<T>::value
    && GraphData::Clearable<T>::value, int>::type>
void GraphData::default_on_reset(Any& data) noexcept {
    auto value = switch_to<T>(data);
    if (value != nullptr) {
        value->clear();
...
inline bool GraphData::bind(ClosureContext& closure) noexcept {
    ...
    if (ABSL_PREDICT_FALSE(!_closure.compare_exchange_strong(expected, &closure,
...
inline bool GraphData::acquire() noexcept {
    bool expected = false;
    if (ABSL_PREDICT_TRUE(_acquired.compare_exchange_strong(expected, true,
                ::std::memory_order_acq_rel))) {
        return true;
    }
    return false;
...
template <>
inline const ::babylon::Any*
GraphData::cvalue<::babylon::Any>() const noexcept {
...
inline OutputData<T> GraphData::declare_type() noexcept {
    if (_declared_type == &::babylon::TypeId<Any>::ID) {
        ...
        BABYLON_LOG(WARNING) << *this
            << " declare type["<< ::babylon::TypeId<T>().get_type_name()
...
template <>
inline ::babylon::Any*
GraphData::mutable_value<::babylon::Any>() noexcept {
    if (ABSL_PREDICT_FALSE(_empty)) {
        return nullptr;
    }
    return &_data;
...
inline bool GraphData::check_last_producer() noexcept {
    return (_producers.empty() || _producer_done_num.fetch_add(1) == (_producers.size() - 1));
}
...
inline int32_t GraphData::activate(DataStack& activating_data,
    ...
    for (auto & producer : _producers) {
        if (ABSL_PREDICT_FALSE(0 != producer->activate(activating_data, runnable_vertexes, closure))) {
            BABYLON_LOG(WARNING) << "activate producer " << producer
                << " of " << *this << " failed";
            return -1;
        }
    }
