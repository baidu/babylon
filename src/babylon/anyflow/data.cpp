...
void GraphData::release() noexcept {
    ...
    } while (ABSL_PREDICT_FALSE(!_closure.compare_exchange_weak(closure,
...
int GraphData::recursive_activate(
...
bool GraphData::check_safe_mutable() const noexcept {
    ...
    for (auto dependency : _successors) {
        if (dependency->inner_condition() == nullptr
                && dependency->is_mutable())  {
            return false;
        }
    }
    // 无法静态判定为一定不安全，先放行
