...
inline constexpr BasicStringView<C, T>::BasicStringView(const_pointer buffer, size_type size) noexcept :
#if __GLIBCXX__
    _size(size), _data(buffer) {}
#else // !__GLIBCXX__
    _data(buffer), _size(size) {}
#endif // !__GLIBCXX__

template <typename C, typename T>
inline constexpr BasicStringView<C, T>::BasicStringView(const_pointer c_str) noexcept :
    BasicStringView(c_str, c_str != nullptr ? traits_type::length(c_str) : 0) {}

template <>
inline constexpr BasicStringView<char, ::std::char_traits<char>>::BasicStringView(const_pointer c_str) noexcept :
    BasicStringView(c_str, c_str != nullptr ? __builtin_strlen(c_str) : 0) {}

template <typename C, typename T>
template <typename A>
inline BasicStringView<C, T>::BasicStringView(const ::std::basic_string<C, T, A>& other) noexcept :
#if __GLIBCXX__
    _size(other.size()), _data(other.data()) {}
#else // !__GLIBCXX__
...
template <typename C, typename T>
inline CONSTEXPR_SINCE_CXX14 void BasicStringView<C, T>::remove_prefix(size_type num) noexcept {
    _data += num;
    _size -= num;
}

template <typename C, typename T>
inline CONSTEXPR_SINCE_CXX14 void BasicStringView<C, T>::remove_suffix(size_type num) noexcept {
    _size -= num;
}

template <typename C, typename T>
inline CONSTEXPR_SINCE_CXX14 void BasicStringView<C, T>::swap(BasicStringView& other) noexcept {
    auto old_data = _data;
    auto old_size = _size;
    _data = other._data;
    _size = other._size;
...
template <typename C, typename T>
inline typename BasicStringView<C, T>::size_type
BasicStringView<C, T>::copy(pointer dest, size_type count, size_type position) const {
    if (position <= size()) {
...
inline constexpr BasicStringView<C, T> BasicStringView<C, T>::substr(
    ...
    return BasicStringView(_data + position, count < _size - position ? count : _size - position);
}

template <typename C, typename T>
inline BABYLON_CONSTEXPR int32_t BasicStringView<C, T>::compare(const BasicStringView& other) const noexcept {
    return _size == other._size ? traits_type::compare(_data, other._data, _size) :
        _size > other._size ? (traits_type::compare(_data, other._data, other._size) < 0 ? -1 : 1) :
            traits_type::compare(_data, other._data, _size) > 0 ? 1 : -1;
}

template <>
inline BABYLON_CONSTEXPR int32_t BasicStringView<char, ::std::char_traits<char>>::compare(
        const BasicStringView<char, ::std::char_traits<char>>& other) const noexcept {
    return _size == other._size ? __builtin_memcmp(_data, other._data, _size) :
        _size > other._size ? (__builtin_memcmp(_data, other._data, other._size) < 0 ? -1 : 1) :
            __builtin_memcmp(_data, other._data, _size) > 0 ? 1 : -1;
}

template <typename C, typename T>
inline BABYLON_CONSTEXPR int32_t BasicStringView<C, T>::compare(size_type position, size_type count,
...
template <typename C, typename T>
inline BABYLON_CONSTEXPR int32_t BasicStringView<C, T>::compare(size_type position, size_type count,
        const_pointer buffer, size_type size) const noexcept {
    return substr(position, count).compare(BasicStringView(buffer, size));
}

template <typename C, typename T>
inline constexpr BasicStringView<C, T>::operator ::std::initializer_list<value_type>() const noexcept {
    return ::babylon::InitializerList<value_type>(_data, _size);
}

#if !__GLIBCXX__
template <typename C, typename T>
inline constexpr BasicStringView<C, T>::operator ::std::string_view() const noexcept {
    return ::std::string_view(_data, _size);
}

template <typename C, typename T>
inline constexpr BasicStringView<C, T>::BasicStringView(::std::string_view sv) noexcept :
        BasicStringView(sv.data(), sv.size()) {}
#endif // !__GLIBCXX__

template <typename C, typename T>
constexpr typename BasicStringView<C, T>::value_type BasicStringView<C, T>::EMPTY;
// BasicStringView end
////////////////////////////////////////////////////////////////////////////////

template <typename C, typename T>
inline constexpr bool operator==(BasicStringView<C, T> left, BasicStringView<C, T> right) noexcept {
    return left.size() == right.size() && 0 == __builtin_memcmp(left.data(), right.data(), sizeof(C) * left.size());
}

template <typename C, typename T>
inline constexpr bool operator==(
        typename ::std::common_type<BasicStringView<C, T>>::type left, BasicStringView<C, T> right) noexcept {
    return left == right;
}
...
template <typename C, typename T>
inline BABYLON_CONSTEXPR bool operator<(BasicStringView<C, T> left, BasicStringView<C, T> right) noexcept {
    return left.compare(right) < 0;;
}
...
inline ::std::basic_ostream<C, T>& operator<<(::std::basic_ostream<C, T>& os,
        BasicStringView<C, T> view) noexcept {
...
struct hash<string_view> {
    ...
    inline __attribute__((always_inline)) size_t operator()(string_view sv) const noexcept {
...
struct equal_to<string_view> {
    // absl和std c++20使用is_transparent定义来判定是否支持异构键
    using is_transparent = void;
    inline __attribute__((always_inline)) bool operator()(string_view left, string_view right) const noexcept {
        return left == right;
    }
};
} // std
...
struct hash<::babylon::StringView> {
    ...
    size_t operator()(::babylon::StringView sv) const noexcept {
#if __GLIBCXX__
        return ::std::_Hash_impl::hash(sv.data(), sv.size());
#else
...
struct equal_to<::babylon::StringView> {
    ...
    using is_transparent = void;
    inline ABSL_ATTRIBUTE_ALWAYS_INLINE
    bool operator()(::babylon::StringView left,
                    ::babylon::StringView right) const noexcept {
        return left == right;
    }
};
} // std
